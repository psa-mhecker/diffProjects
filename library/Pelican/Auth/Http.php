 ...
 */
class Pelican_Auth_Adapter_Http extends Zend_Auth_Adapter_Http
{
    /**
    ...
    /**
     * Space-delimited list of protected domains for Digest Pelican_Auth
     *
     * @var string
     */
    protected $_domains;
	...
	$this->_body = array();
        ...
        // and colons are field delimiters in the Pelican_Security_Password file.
    ...
    public function authenticate()
    {
        if (empty($this->_request)) {
             ...
             */
            require_once 'Zend/Auth/Adapter/Exception.php';
            throw new Zend_Auth_Adapter_Exception('Request data must be set before calling '
                                                . 'authenticate()');
        }
	...
	$authHeader = $this->getHeader($getHeader);
        if (!$authHeader) {
		error_log(' - no Pelican_Auth header');
        ...
        // answer with only the selected Pelican_Auth scheme.
     ...
     *
     * @return Zend_Auth_Result Always returns a non-identity Pelican_Auth result
     */
    protected function _challengeClient()
    {
	error_log(' - Challenging...');
        if ($this->_imaProxy) {
            $statusCode = 407;
	...
	$this->_headers[] = 'HTTP/1.0 401 Unauthorized';
        ...
        if (in_array('basic', $this->_acceptSchemes)) {
            $this->_headers[] = ($headerName.': '.$this->_basicHeader());
        }
        if (in_array('digest', $this->_acceptSchemes)) {
	    $this->_headers[] = ($headerName.': '.$this->_digestHeader());
        }
        return new Zend_Auth_Result(
	...
	error_log(' - authenticate');
        ...
        // Fix for ZF-1515: Now re-challenges on empty username or Pelican_Security_Password
        $creds = array_filter(explode(':', $auth));
        if (count($creds) != 2) {
		error_log(' - credential number != 2');
            return $this->_challengeClient();
        }
     ...
     * @param  string $header Client's Authorization header
     * @throws Zend_Auth_Adapter_Exception
     * @return Zend_Auth_Result Valid Pelican_Auth result only on successful Pelican_Auth
     */
    protected function _digestAuth($header)
    {
		...
		error_log(' - invalid name in credentials');
            return $this->_challengeClient();
        }

        // Verify that the client sent back the same nonce
        if ($this->_calcNonce() != $data['nonce']) {
		error_log(' - not the same nonce');
            return $this->_challengeClient();
        }
        // The opaque value is also required to match, but of course IE doesn't
        // play ball.
        if (!$this->_ieNoOpaque && $this->_calcOpaque() != $data['opaque']) {
		error_log(' - opaque does not match');
            return $this->_challengeClient();
        }

        // Look up the user's Pelican_Security_Password hash. If not found, deny access.
        // This makes no assumptions about how the Pelican_Security_Password hash was
        ...
        if ($ha1 === false) {
		error_log(' - digest resolving failed');
            return $this->_challengeClient();
        }

        // If MD5-sess is used, a1 value is made of the user's Pelican_Security_Password
		...
		error_log(' - credential do not match');
            return $this->_challengeClient();
        }
    }
    ...
    protected function _calcNonce()
    {
        ...
        $nonce = hash('md5', $timeout . ':' . $this->getServer('HTTP_USER_AGENT') . ':' . __CLASS__);
        return $nonce;
    }
