...
class Pelican_Text {
    ...
    */
    ...
    public static function utf8entities($str) {
        if (!is_string($str)) die('<b>Warning:</b><br/>' . '<tt>utf8entities(string $source)</tt> : $source should be a string.');
        //utf8 to unicode
        $unicode = array();
        $values = array();
        $lookingFor = 1;
        ...
        $entities = '';
        foreach($unicode as $value) $entities.= ($value < 128 ? chr($value) : ('&#' . $value . ';'));
        return $entities;
    }
