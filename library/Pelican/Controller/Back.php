...
class Pelican_Controller_Back extends Pelican_Controller
{
    ...
    protected function _assignVars ($paramList)
    {
        foreach ($paramList as $key => $get) {
            ...
            if (empty($this->$key)) {
                 ...
                 */
                /*
                 * $this->$key = $this->getRequest()->getParam('post', $get); $this->getRequest()->setParam('get', $get, $this->$key); $_GET[$get] = $this->$key;
                 */
    ...
    public function stopStandardForm ()
    {
         ...
         */
        $sRetour = '';
        // $sRetour = Pelican::$config ["PAGE_INDEX_IFRAME_PATH"] . "?tid=" . $_GET ["tid"] . "&tc=" . $_GET ["tc"] . "&view=" . $_GET ["view"];
    ...
    protected function beginForm (&$oForm)
    {
        ...
        $this->last = $this->aLastValues; // / @todo refactoring
    }
    ...
    {
        if ($value || $value === 0) {
            $this->aDefaultValues[$param] = $value;
        }
    }
    ...
    protected function getDefaultField (&$oForm, $retour = "")
    {
        ...
        $form .= $oForm->createHidden("form_schedule", "");
    ...
    protected function getTemplateTitle ($title, $subtitle = "", $class = "form_title")
    {
        ...
        if (! $this->bPopup && ! $this->print) {
            if (! empty($_GET["is_edit_front"])) {
                ...
                ), "if(parent.setRightTitle) parent.setRightTitle('" . (addslashes(Pelican_Text::unhtmlentities($title))) . "')");
            } else {
                $return = Pelican_Html::script(array(
                    type => "text/javascript"
                ), "if(top.setRightTitle) top.setRightTitle('" . (addslashes(Pelican_Text::unhtmlentities($title))) . "')");
            }
