...
class Pelican_Controller_Image extends Pelican_Controller
{
    ...
    public function formatAction ()
    {
        ...
        $image = new Pelican_Cache_Media((! empty($params['path']) ? $params['path'] : ''), (! empty($params["format"]) ? $params["format"] : ''), (! empty($params["crop"]) ? $params["crop"] : ''), (! empty($params["bypass"]) ? true : false), (! empty($params['life']) ? $params['life'] : ''), (! empty($imageGab["MEDIA_FORMAT_COMPLETE_COLOR"]) ? $imageGab["MEDIA_FORMAT_COMPLETE_COLOR"] : ''), (! empty($params['width']) ? $params['width'] : ''), (! empty($params['height']) ? $params['height'] : ''), (! empty($params['extension']) ? $params['extension'] : ''));
        ...
        if (! empty($image->value)) {
            if (! empty($store)) {
                ...
                $this->getRequest()->setHeaders('Content-Type', Pelican::$config["IM_MIME"]);
