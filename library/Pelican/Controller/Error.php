...
class Pelican_Controller_Error extends Pelican_Controller
{
    ...
    {
        $this->_message = $this->getParam('msg');
	...
	*/
    ...
    public function code404Action()
    {
        $head = $this->getView()->getHead();
        $head->setTitle("404");
        $this->assign('body', (!empty($this->_message) ? $this->_message : 'PAGE NOT FOUND'));
    ...
    public function code500Action()
    {
        $head = $this->getView()->getHead();
        $head->setTitle("500");
        $this->assign('body', (!empty($this->_message) ? $this->_message : 'INTERNAL SERVER ERROR'));
        $this->fetch();
    }
