...
class Pelican_Security {
     ...
     *
     * @access public
     * @param mixed $aSecurity (option) Liste des typages de paramètres ("param"=>
    ...
    public static function checkLFI()
    {
        ...
        /** on traite GET et POST en meme temps */
        ...
        $control = array('etc/passwd', 'vcombined_log', 'phpinfo()', '<?php', '.htaccess', '.bash_history', 'error_log', 'ssl_request_log', '/init.d/', 'httpd.conf', 'proc/self/environ', 'php://', '/cmd', '\cmd', 'config.php', 'config.php');
        ...
        $check = str_replace($controlGet, '', $get);
        if ($controlGet != $check) {
        $cleanRequest = '';
        }
        */
            ...
            foreach(Pelican::$config['server_configuration'] as $key => $value) {
                ...
                if ($val != $value) {
                    if (!empty($_GET['checkconfig'])) {
                        var_dump('probl�me de configuration serveur "' . $key . '" : "' . $value . '" attendu, "' . $val . '" identifi�');
                    }
                    ini_set($key, $value);
                }
            }
            ...
            if (is_array(Pelican::$config['server_extension'])) {
                foreach(Pelican::$config['server_extension'] as $extension) {
                    if (!extension_loaded($extension)) {
                        var_dump('probl�me de configuration serveur : extension "' . $extension . '" manquante');
                    }
                }
            }
            die();
     ...
     *
     * @access public
     * @param mixed $vars Tableau de paramètre à contrôler et à typer
     * @param mixed $signatures Tablerau de paramétrage défini dans
     * /application/configs/security.ini.php
     * @param bolean $securityFunction (option) Function d'echappement des paramètres
     * inconnus
     * @return void
     */
                ...
                if ($securityFunction == "upload") {
                    ...
                    if ($sig) {
                        ...
                        if (isset($sig['type'])) {
                            ...
                            Pelican_Log::control('Variable non typ�e : ' . $vars[$name], 'security');
                        }
     ...
     *
     * @access public
     * @param mixed $value Texte
     * @param string $function (option) Fonction d'échappement : en général
    ...
    public static function checkExtension($file)
    {
        ...
        return !in_array($pathinfo['extension'], array('php', 'sh', 'bat', 'exe', 'com'));
    }
     ...
     * @param string $redirect (option) Page de redirection en cas d'échec
     ...
     *
     * @access public
     * @param string $output Code Pelican_Html à traiter
     * @param bool $compressOutput (option) Compresser la sortie
     * @param bool $cleanComment (option) Supprimer les commentaires
     * @param bool $encodeEmail (option) Encoder les emails
     * @return string
     */
    ...
    public static function inputToken()
    {
        ...
        echo Pelican_Html::input(array(type => "hidden", name => "token", value => $token));
    }
     ...
     *
     * @access public
     * @param int $duration (option) Durée de validité du Token
     * @return bool
     */
     ...
     * @param string $type (option) Captcha type
    ...
    public static function inputCaptcha($label = "", $type = 'STANDARD', $options = array())
    {
        switch ($type) {
            case "RECAPTCHA":
                ...
                $recaptcha = new Zend_Service_ReCaptcha(Pelican::$config['security']["RECAPTCHA"]["PUBLICKEY"], Pelican::$config['security']["RECAPTCHA"]["PRIVATEKEY"]);
            ...
            break;
            case "STANDARD":
                $return = Pelican_Html::img(array(width => 200, height => 60, alt => "Image de contrôle", style => "float:right;vertical-align: middle;", src => "/library/Pelican/Security/public/captcha/visual-captcha.php", border => "1"));
                $return.= $label;
                $return.= Pelican_Html::input(array(type => "text", name => "userdigit", id => "userdigit", size => 10));
                return Pelican_Html::div(array(style => "width:500px;"), $return);
            break;
        }
    }
     ...
     *
     * @access public
     * @param string $IP (option)
     * @param string $Host (option)
     * @param string $Server (option)
     * @return string
     */
     ...
     *
     * @access public
     * @param string $plageIP Page d'IP
     * @return string
     */
     ...
     *
     * @access public
     * @param bool $state (option) Activer ou non
     * @param bool $show (option) Afficher le résultat à l'écran
     * @return void
     */
     ...
     *
     * @access public
     * @param string $text Valeur à contrôler
     * @return string
     */
 ...
 *
 * @param string $val Valeur à contrôler
 * @return string
 */
