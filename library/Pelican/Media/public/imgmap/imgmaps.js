...
var xoxG='Mode Edition. Ajouter de nouvelles zones ou modifier celles qui existent.';
...
function getMapHTML(){
  ...
  var ztid='';
  if(window.dialogArguments["ztid"]) ztid=window.dialogArguments["ztid"];
  var mapname=ztid;
  html='<map id="img_map_'+mapname+'" name="img_map_'+mapname+'">'+html+'</map>';
	return(html);
};
function setMapHTML(html){
	...
	var newareas=div.getElementsByTagName('area');for(var i=0;i<newareas.length;i++){
		...
		newareas[i].attributes["href"].value=newareas[i].attributes["href"].value.replace(/about:/,"");
...
function togglePreview(){
	if (viewmode==0){
		...
		preview.innerHTML=getMapHTML();
    pic.setAttribute('useMap','#'+mapname);
    pic.onmousedown=null;pic.onmousemove=null;pic.style.cursor='auto';viewmode=1;document.getElementById('i_preview').src=srcroot+'edit.gif';statusMessage(xoxQ);
	} else{
		for(var i=0;i<areas.length;i++){
...
function addNewArea(){
	...
	props[id].innerHTML='\
			<input type="text"  name="img_id" class="img_id" value="'+(id+1)+'" readonly="1"/>\
			...
			Lien : <input type="text" MAXLENGTH="80" name="img_href" id="img_href'+id+'" class="img_href" value=""><img src="images/internal_link.gif" alt="Lien interne" title="Lien interne" width="23" height="22" border="0" align="middle" style="cursor:pointer" onclick="return popupInternalLink(document.getElementById(\'img_href'+id+'\'))">\
			Info-bulle : <input type="text" MAXLENGTH="60" name="img_alt" class="img_alt" value="">\
...
function removeArea(id){
	...
	if (props[id]){
		...
		pprops.removeChild(props[id]);props[id]=null;
...
function img_area_blur(){
    ...
    if(document.getElementById('img_area_'+id)){
        xoxf(id);area_html.value=getMapHTML();
    }
};
