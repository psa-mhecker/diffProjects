...
function moveTop(value)
{
	...
	if (parseInt(z.style.top +0) > <?=$imageSize[1]?> - parseInt(z.offsetHeight +0)) {
		z.style.top = (<?=$imageSize[1]?> - parseInt(z.offsetHeight + 0))+"px";
	}
...
function moveLeft(value)
{
	...
	if (parseInt(z.style.left + 0) > <?=$imageSize[0]?> - parseInt(z.offsetWidth +0)) {
		z.style.left = (<?=$imageSize[0]?> - parseInt(z.offsetWidth + 0))+"px";
	}
...
function move(e)
{
	...
	if (source.id == "container" || source.id == "selection") {
		if (dragapproved){
			z.style.top = (ns6 ? temp2 + e.clientY - y : temp2 + e.clientY - y) + "px";
			moveTop(0);
			z.style.left = (ns6 ? temp1 + e.clientX - x : temp1 + e.clientX - x) + "px";
			moveLeft(0);
