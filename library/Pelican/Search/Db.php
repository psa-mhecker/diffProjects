...
class Pelican_Search_Db implements Pelican_Search_Interface
{
    ...
    public function getStatistics($site, $langue, $fields, $filters)
    {
        ...
        if ($_GET["recDate1"]) $research["recDate1"] = $_GET["recDate1"];
        if ($_GET["recDate2"]) $research["recDate2"] = $_GET["recDate2"];
        if ($_GET["recChamp"]) $research["recChamp"] = $_GET["recChamp"];
        if ($_GET["recLang"]) $research["recLang"] = $_GET["recLang"];
        if ($_GET["recCategory"]) $research["recCategory"] = $_GET["recCategory"];
            ...
            foreach ($filters as $params) {
                ...
                if ($params[1]) {
                    switch ($params[2]) {
                        ...
                        case "list": {
                                $list = array();
                                if (!is_array($params[1])) $params[1] = array($params[1]);
                                $j = 0;
         ...
         * @return mixed
         */
        function getResult($current_page = 1, $nbResultPerPage = 10, $order = "date", $fields = array())
        {
            global $sSQLWHERE, $aBindSearch, $aScore;
            $oConnection = Pelican_Db::getInstance();
         ...
         * @return __TYPE__
         */
        function containsClause($aField, $BindKey, &$aBind, $join = "OR", $order = 1)
        {
            $oConnection = Pelican_Db::getInstance();
            if ($aField) {
         ...
         * @return __TYPE__
         */
        function matchClause($sField, $BindKey, &$aBind, $join = "OR", $order = 1)
        {
            $oConnection = Pelican_Db::getInstance();
            if ($sField) {
                if ($oConnection->info["type"] == "MySQL") {
