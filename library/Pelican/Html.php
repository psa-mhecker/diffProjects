...
class Pelican_Html
{
    ...
    // special attribute: 'more_attr'=>$events (unformatted string)
    ...
    */
    ...
    // self::fieldset(self::legend(self::label(array(ACCESSKEY=>'V'),Pelican_Html_Input::text(...)),
    //                       self::label(array(ACCESSKEY=>'V'), ...))))
    ...
    public static function flash()
    {
        ...
        if (is_array($args[0])) {
            ...
            //			if (!$attr['bgcolor']) $attr['bgcolor'] = "#ffffff";
            //			if (!$attr['width']) $attr['width'] = "100%";
            //			if (!$attr['height']) $attr['height'] = "100%";
            if (!$attr['scale']) {
                $attr['scale'] = "noscale";
            }
            if (!$attr['wmode']) {
     ...
     * @param __TYPE__ $width __DESC__
     * @param __TYPE__ $height __DESC__
     * @param __TYPE__ $params (option) __DESC__
     * @param __TYPE__ $comment (option) __DESC__
     * @return __TYPE__
     */
   ...
   so.write("swfObj_' . $id . '");';
    ...
    // Pelican_Html_Input::hidden shortcut
    // hidden($name,$value)
    ...
    {
        return Pelican_Html_Input::hidden(array('name' => $name, 'value' => $value));
    }
    // internal functions
     ...
     * @return __TYPE__
     */
    public static function _tag1($tag, $args = false, $cdata = false)
    {
        if (!$args) {
            return '<' . $tag . '></' . $tag . '>';
        } else {
        ...
        if (!empty($attr)) {
            ...
            foreach ($attr as $key => $value) {
                ...
                if ($key == 'more_attr') {
                    ...
                    if ($value) {
                        // @deprecated						$tag .= (XHTML ? $key.'="'.$key.'" ' : $key.' ');
                        $tag .= $key . '="' . $key . '" ';
                    }
                } else {
     ...
     * @access private
     * @param __TYPE__ $tag __DESC__
     * @param __TYPE__ $def_name __DESC__
     * @param __TYPE__ $attr __DESC__
     * @param bool $args (option) __DESC__
     * @return __TYPE__
     */
     ...
     * @access private
     * @param __TYPE__ $tag __DESC__
     * @param __TYPE__ $overrides __DESC__
     * @param bool $args (option) __DESC__
     * @return __TYPE__
     */
    public static function _attr_overrides($tag, $overrides, $args = false)
    {
...
class Pelican_Html_Input extends Pelican_Html
{
    ...
    public static function radio()
    {
        ...
        return call_user_func_array(array($this, '_attr_overrides'), array('input', array('type' => 'radio'), $args));
    }
    // Pelican_Html_Input::hidden(array(name=>$name,value=$value))
    ...
    public static function hidden()
    {
        ...
        return self::_attr_overrides('input', array('type' => 'hidden'), $args);
    }
    // Pelican_Html_Input::text(array(name=>$name,value=$value,size=>$size,'class'=>$class,...))
