ContextMenu.initializeContextMenu=function()
{
	//a refaire pour firefox
	document.body.insertAdjacentHTML("BeforeEnd", '<iframe src="/library/blank.html" scrolling="no" class="WebFX-ContextMenu" marginwidth="0" marginheight="0" frameborder="0" style="position:absolute;display:none;z-index:50000000;" id="WebFX_PopUp"></iframe>');
	...
	if (WebFX_PopUpcss) {
		...
		self.onblur=function(){WebFX_PopUpcss.style.display="none"};
	}
}
...
ContextMenu.populatePopup=function(arr,win)
{
	...
	if (doc.getElementsByTagName("LINK").length == 0) {
		doc.open();
		doc.write('<html><head><link rel="StyleSheet" type="text/css" href="' + libDir + '/External/context/css/WebFX-ContextMenu.css"></head><body></body></html>');
		doc.close();
	}
	for(i=0;i<alen;i++)
	{
