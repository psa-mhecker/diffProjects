...
define("PERCENT_MOBILE_VERSION", "260610_php");
...
function percent_mobile_track($site_id,$url="") {	
	...
	$percent_mobile_referer="";

	if (isset($_SERVER['HTTP_REFERER']))
	{
		$percent_mobile_referer=$_SERVER['HTTP_REFERER'];
	}	
	...
	if(isset($_SERVER['HTTP_USER_AGENT'])&&preg_match("/\(iPhone/",$_SERVER['HTTP_USER_AGENT'])) {
		$image_url=((isset($_SERVER['HTTPS'])&&$_SERVER["HTTPS"]=="on")?"https":"http")."://tracking.percentmobile.com/pixel/".$site_id."/".rand(0, 0xffff).".gif?v=".PERCENT_MOBILE_VERSION."&us=".$percent_mobile_u."&vi=".$percent_mobile_v."&url=".urlencode($url)."&referer=".urlencode($percent_mobile_referer);			
