...
(function() {
	tinymce.create('tinymce.plugins.betd_save', {
		...
		_save : function() {
			...
			ed.setProgressState(50); // Hide progress
			...
			if (formObj) {
				tinyMCE.triggerSave();
				// remplacement du 1er <p>&nbsp;</p> ajouté par Tiny au chargement -> à corriger plus tard si possible
				var sStartContent = new String(tinymce.trim(ed.getContent()));
				if (sStartContent.substring(0,13) == '<p>&nbsp;</p>') {
					sStartContent = sStartContent.substring(13,sStartContent.length);
				}
				ed.startContent = sStartContent;
				window.opener.getElementById(ed.getParam('boxName')).value=t.getMediaVarPath(ed.startContent);
				t.generatePreview();
				window.bEditorSaved = true;
				window.close();
