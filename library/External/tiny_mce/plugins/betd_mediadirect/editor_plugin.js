...
(function() {
	...
	tinymce.create('tinymce.plugins.betd_media_directPlugin', {
		init : function(ed, url) {
			...
			// Register commands
			ed.addCommand('betd_mediadirect', function() {
				ed.windowManager.open({
					file : url + '/media.htm',
					width : 430 + parseInt(ed.getLang('betd_mediadirect.delta_width', 0)),
					height : 160 + parseInt(ed.getLang('betd_mediadirect.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url
				});
			...
			// Register buttons
			ed.addButton('betd_mediadirect', {title : 'betd_mediadirect.desc', cmd : 'betd_mediadirect',image : url + '/img/mediadirect.gif'});
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
						if (e.nodeName == 'IMG' && /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(e.className)) {
							m.add({title : 'betd_mediadirect.edit', icon : 'betd_mediadirect', cmd : 'betd_mediadirect'});
						}
					});
				}
			});
		...
		getInfo : function() {
			return {
				longname : 'Media Direct Upload ',
				author : 'business decision patrick.deroubaix@businessdecision.fr',
				authorurl : 'www.businessdecision.com',
	...
	// Register plugin
	tinymce.PluginManager.add('betd_mediadirect', tinymce.plugins.betd_media_directPlugin);
})();