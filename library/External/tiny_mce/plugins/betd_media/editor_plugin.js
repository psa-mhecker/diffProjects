...
(function() {
	...
	tinymce.PluginManager.requireLangPack('betd_media');

	tinymce.create('tinymce.plugins.betd_media', {
		init : function(ed, url) {
			...
			// Register commands
			ed.addCommand('betd_image', function() {
				ed.windowManager.open({
					file : url + '/popup_media.php?mediaType=image&mediaZone=editor',
					width : 660,
					height : 430,
					inline : 1
				}, {
			...
			ed.addCommand('betd_file', function() {
				ed.windowManager.open({
					file : url + '/popup_media.php?mediaType=file&mediaZone=editor',
					width : 660,
					height : 430,
					inline : 1
				}, {
			...
			ed.addCommand('betd_flash', function() {
				ed.windowManager.open({
						file : url + '/popup_media.php?mediaType=flash&mediaZone=editor',
						width : 660,
						height : 430,
						inline : 1
				}, {
			...
			// Register buttons
			ed.addButton('betd_image', {title : 'betd_media.image', cmd : 'betd_image', image : url + '/img/image.gif'});
			ed.addButton('betd_file', {title : 'betd_media.file', cmd : 'betd_file', image : url + '/img/file.gif'});
			ed.addButton('betd_flash', {title : 'betd_media.flash', cmd : 'betd_flash',image : url + '/img/flash.gif'});

			ed.onNodeChange.add(function(ed, cm, n, co) {
				cm.setDisabled('betd_file', co && n.nodeName != 'A');
				cm.setActive('betd_file', n.nodeName == 'A' && !n.name);
				cm.setActive('betd_flash', n.nodeName == 'IMG' && isMediaElm(n));
			});
...
// pour le flash
			...
			ed.onBeforeSetContent.add(function(ed, o) {
				...
				h = h.replace(/<script[^>]*>\s*write(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)\(\{([^\)]*)\}\);\s*<\/script>/gi, function(a, b, c) {
					...
					return '<img class="mceItem' + b + '" title="' + ed.dom.encode(c) + '" src="' + url + '/img/flash_tmp.gif" width="' + o.width + '" height="' + o.height + '" />'
				});
					...
					each(dom.select('IMG', o.node), function(n) {
						...
						switch (n.className) {
							...
							case 'mceItemQuickTime':
								...
								cb = 'http://www.apple.com/qtactivex/qtplugins.cab#version=6,0,2,0';
								mt = 'video/quicktime';
								break;
		...
		_buildObj : function(o, n) {
			...
			each (p, function(v, k) {
				if (v && !/^(width|height|codebase|classid)$/.test(k))
					dom.add(ob, 'span', {mce_name : 'param', name : k, 'value' : v}); 
			});
	...
	// Register plugin
	tinymce.PluginManager.add('betd_media', tinymce.plugins.betd_media);
})();