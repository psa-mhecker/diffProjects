...
class Cpw_GRCOnline_CustomerAt_User
{
    ...
    private function SetProfileDatas($profileDatas, $email)
    {
    	...
    	if (isset($profileDatas[Cpw_GRCOnline_Customerfields::USR_GENDER]))
    	{
    		$this->Gender = $profileDatas[Cpw_GRCOnline_Customerfields::USR_GENDER];
    	}
    	if (isset($profileDatas[Cpw_GRCOnline_Customerfields::USR_CIVILITY]))
    	{
    		$this->Civility = $profileDatas[Cpw_GRCOnline_Customerfields::USR_CIVILITY];
    	}
    	
    	foreach ($fields as $field)
    	{
    		if(defined('Cpw_GRCOnline_Customerfields::'.$field))
    		{
    ...
    function compare_vehicle(Cpw_GRCOnline_CustomerAt_Vehicle $a, Cpw_GRCOnline_CustomerAt_Vehicle $b) 
    {
    ...
    {
    	$customerMng 	= new Cpw_GRCOnline_Customermanager(PUBLIC_PATH.'/Wsdl/CRMDirect.wsdl');
	    $this->getDataCustomer($customerMng, $email, $culture);
	    if ($customerMng->onError()) return false;
	    
	    $xmlloader = new Cpw_GRCOnline_Customerxmlloader ($customerMng->data());
		...
		if (count($vehicles)>0) 
		{
			...
			{
				$_vehcus = new Cpw_GRCOnline_CustomerAt_Vehicle($vehicle);
			...
			usort($this->AllVehicles, array("Cpw_GRCOnline_CustomerAt_User", "compare_vehicle"));
		...
		if (count($dealers)>0)
		{
			...
			{
				$_dealercus = new Cpw_GRCOnline_CustomerAt_Dealer($dealer);
				if ($_dealercus->Type == 'relatedgeositepreferredvn') $this->DealerVN[] = $_dealercus;
				if ($_dealercus->Type == 'relatedgeositepreferredapv') $this->DealerAPV[] = $_dealercus;
			}
		}
		...
		if (count($subscriptions)>0)
		{
			...
			{
				$this->Subscriptions[] = new Cpw_GRCOnline_CustomerAt_Subscription($subscription);
			}
		}
