...
$.widget( "evol.colorpicker", {
	...
	options: {
		...
		strings: 'Theme Colors,Standard Colors,Web Colors,Theme Colors,Back to Palette,History,No history yet.,Standard Colors2',
		standardColor: false,
		color1: standardColors,
		color2: standardColors,
		color3: standardColors,
	},
	...
	_paletteHTML: function() {
		...
		var h='<div class="evo-pop'+_ie+' ui-widget ui-widget-content ui-corner-all"'+
			...
			// links
			if(opts.hideWebColor){
				h+='<div class="evo-more"><a href="javascript:void(0)">'+labels[1+pIdx]+'</a>';
			}else{
				h+='<br>';
			}
	...
	_paletteHTML1: function() {
		...
		if(opts.color1.length > 0) {
			for (i = 0; i < opts.color1.length; i++) {
				if (jumpI == 10) {
					h += '<tr><th colspan="10"></th></tr>';
					jumpI = 0;
				}
				h += oTD + opts.color1[i] + cTD;
				jumpI++;
			}
		}
		...
		var jumpI = 0;
		// standard colors
		if(opts.color2.length > 0) {
			for (i = 0; i < opts.color2.length; i++) {
				if (jumpI == 10) {
					h += '<tr><th colspan="10"></th></tr>';
					jumpI = 0;
				}
				h += oTD + opts.color2[i] + cTD;
				jumpI++;
			}
		}
		h+='</tr>'+oTRTH;
		...
		var jumpI = 0;
		// Color 3
		if(opts.color3.length > 0) {
			for (i = 0; i < opts.color3.length; i++) {
				if (jumpI == 10) {
					h += '<tr><th colspan="10"></th></tr>';
					jumpI = 0;
				}
				h += oTD + opts.color3[i] + cTD;
				jumpI++;
			}
		}
		h+='</tr></table>';
