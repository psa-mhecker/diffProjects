<?php
	...
	* @version 1.13 2003/02/07 20:44:09
	*/
...
//  function create($p_filelist, $p_add_dir="", $p_remove_dir="")
  function create($p_filelist /*, options */)
  {
    ...
    if ($v_size > 1) {
      ...
      // ----- Remove Pelican_Form the options list the first argument
...
//  function add($p_filelist, $p_add_dir="", $p_remove_dir="")
  ...
  //     Pelican_Index_Comment : Pelican_Index_Comment associated with the file
  //     folder : true | false
  //     Pelican_Index : Pelican_Index of the file in the archive
  ...
  //   The extracted files or folders are identified by their Pelican_Index in the
  //   archive (from 0 to n).
  //   Note that if the Pelican_Index identify a folder, only the folder entry is
  //   extracted, not all the files included in the archive.
  // Parameters :
  //   $p_index : A single Pelican_Index (integer) or a string of indexes of files to
  //              extract. The Pelican_Form of the string is "0,4-6,8-12" with only numbers
...
//  function extractByIndex($p_index, $p_path="./", $p_remove_path="")
  function extractByIndex($p_index /* $options */)
  {
    ...
    // ----- Look if the Pelican_Index is a string
  ...
  //   The entries (files or folders) are identified by their Pelican_Index in the
  //   archive (0-n).
  //   Note that if the Pelican_Index identify a folder entry, only the folder entry
  //   is deleted, and not all the files included in the folder.
  // Parameters :
  //   $p_index : A single Pelican_Index (integer) or a string of indexes of files to
  //              delete. The Pelican_Form of the string is "0,4-6,8-12" with only numbers
  //              and '-' for range or ',' to separate ranges. No spaces or ';'
  //              are allowed.
  // Return Values :
  //   0 on failure,
  ...
  //     Pelican_Index_Comment : Pelican_Index_Comment associated with the archive file
  //     status : not_exist, ok
  // Parameters :
  //   None
  // Return Values :
...
// --------------------------------------------------------------------------------
  ...
  function privParseOptions(&$p_options_list, $p_size, &$v_result_list, $v_requested_options)
  {
    ...
    $i=0;
    while ($i<$p_size) {
      //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 4, "Looking for table Pelican_Index $i, option = '".PclZipUtilOptionText($p_options_list[$i])."(".$p_options_list[$i].")'");
    ...
    */
    ...
    if (is_file($p_filename))
    {
      ...
      while ($v_size != 0)
      {
        ...
        $v_binary_data = Pelican_Index_Pack('a'.$v_read_size, $v_buffer);
  ...
  function privWriteFileHeader(&$p_header)
  {
    ...
    // ----- Packed data
    $v_binary_data = Pelican_Index_Pack("VvvvvvVVVvv", 0x04034b50, $p_header['version'], $p_header['flag'],
  ...
  function privWriteCentralFileHeader(&$p_header)
  {
    ...
    // ----- Packed data
    $v_binary_data = Pelican_Index_Pack("VvvvvvvVVVvvvvvVV", 0x02014b50, $p_header['version'], $p_header['version_extracted'],
  ...
  function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
  {
    ...
    // ----- Packed data
    $v_binary_data = Pelican_Index_Pack("VvvvvVVv", 0x06054b50, 0, 0, $p_nb_entries, $p_nb_entries, $p_size, $p_offset, strlen($p_comment));
  ...
  //     $p_info['comment'] = Pelican_Index_Comment associated with the file.
  ...
  function privExtractByIndex(&$p_file_list, $p_index, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
    ...
    // ----- Manipulate the Pelican_Index list
    $p_index = strtr($p_index, ' ', '');
    //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Reduced Pelican_Index : '$p_index'");
    ...
    for ($i=0, $j_start=0, $v_nb_extracted=0; ($i<$v_central_dir['entries']) && ($j_start<sizeof($v_index_list)); $i++)
    {
      ...
      // ----- Look if the Pelican_Index is in the list
      $v_extract = false;
      for ($j=$j_start; ($j<sizeof($v_index_list)) && (!$v_extract); $j++)
      {
        //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Look if Pelican_Index '$i' is in '".$v_index_list[$j]."'");
        ...
        else if ($v_size_item_list == 2)
        {
          ...
          {
            //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "Found as part of an Pelican_Index range");
            $v_extract = true;
          }
          if ($i>=$v_item_list[1])
          {
      ...
      if ($v_extract)
      {
        //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Extracting file '".$v_header['filename']."', Pelican_Index '$i'");
  ...
  function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
    ...
    if (!($p_entry['external']==0x41FF0010))
    {
      ...
      else
      {
        ...
        $v_binary_data = Pelican_Index_Pack('va1a1Va1a1', 0x8b1f, Chr($p_entry['compression']), Chr(0x00), time(), Chr(0x00), Chr(3));
        ...
        $v_binary_data = Pelican_Index_Pack('VV', $p_entry['crc'], $p_entry['size']);
  ...
  function privDeleteByIndex($p_index, &$p_result_list)
  {
    ...
    for ($i=0, $j_start=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++)
    {
      ...
      if ($v_found)
      {
        //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "File '".$v_header['filename']."', Pelican_Index '$i' need to be deleted");
        unset($v_header_list[$v_nb_extracted]);
      }
      else
      {
        //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "File '".$v_header['filename']."', Pelican_Index '$i' will not be deleted");
        $v_nb_extracted++;
      }
    }
