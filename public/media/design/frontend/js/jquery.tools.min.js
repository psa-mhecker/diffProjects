...
;(function($){
	...
	$.fn.bxSlider = function(options){
		...
		var start = function(){
			...
			if (slider.settings.touchEnabled && !slider.settings.ticker && ISTOUCH) initTouch();

			
			el.bind('redrawSlider', el.redrawSlider);
		}
		...
		var initTouch = function(){
			...
			if(testMsGesture){
				slider.viewport.addClass('slider-no-touch-action');
				gesture.target = slider.viewport[0];
				slider.viewport[0].addEventListener('MSPointerDown', onTouchStart, false);
			} else {
				slider.viewport.bind(STARTEVENT, onTouchStart);
			}
		}
		...
		var onTouchStart = function(e){
			...
			if(slider.working){
				e.preventDefault();
			} else if(testMsGesture){
				slider.touch.originalPos = el.position();
				slider.touch.start.x = e.clientX;
				slider.touch.start.y = e.clientY;

				slider.viewport[0].addEventListener('MSPointerMove',onTouchMove,false);
				slider.viewport[0].addEventListener('MSPointerUp',onTouchEnd,false);
			}else{
				...
				slider.viewport.bind(MOVEEVENT, onTouchMove);
				// bind a "touchend" event to the viewport
				slider.viewport.bind(ENDEVENT, onTouchEnd);
			}
		}
		...
		var onTouchMove = function(e){
			...
			//modifie10
			var xMovement = Math.abs((testMsGesture)? e.clientX : orig.changedTouches[0].pageX - slider.touch.start.x);
			var yMovement = Math.abs((testMsGesture)? e.clientY : orig.changedTouches[0].pageY - slider.touch.start.y);

			// PREVENT SURFACE TO DISABLE LINK ON SLIDE

			//modifie10
			slider.touch.end.x = (testMsGesture)? e.clientX : orig.changedTouches[0].pageX;

			var distance = Math.abs(slider.touch.start.x - slider.touch.end.x);

			if(testMsGesture && distance < 50){ 
				return false;
			}
			...
			if(slider.settings.mode != 'fade' && slider.settings.oneToOneTouch){
				...
				if(slider.settings.mode == 'horizontal'){
					...
					//modifie10
					var change = (testMsGesture)? e.clientX - slider.touch.start.x : orig.changedTouches[0].pageX - slider.touch.start.x;
					value = slider.touch.originalPos.left + change;
				// if vertical, drag along y axis
				}else{

					//modifie10
					var change = (testMsGesture)? e.clientY - slider.touch.start.y : orig.changedTouches[0].pageY - slider.touch.start.y;
					value = slider.touch.originalPos.top + change;
				}
				setPositionProperty(value, 'reset', 0);
			}
			$(slider.viewport.context).trigger('onTouchMove');
		}
		...
		var onTouchEnd = function(e){
			if(testMsGesture){ 
				slider.viewport[0].removeEventListener('MSPointerMove', onTouchMove, false); 
			} else {
				slider.viewport.unbind(MOVEEVENT, onTouchMove);
				var orig = e.originalEvent;
			}
			var value = 0;
			...
			//modifie10
			slider.touch.end.x = (testMsGesture)? e.clientX : orig.changedTouches[0].pageX;
			slider.touch.end.y = (testMsGesture)? e.clientX : orig.changedTouches[0].pageY;
			...
			slider.viewport.unbind(ENDEVENT, onTouchEnd);
			if(testMsGesture){ slider.viewport[0].removeEventListener('MSPointerUp', onTouchMove, false); }
		}
	...
	$.fn.jScrollPane = function(settings)
	{
		...
		function JScrollPane(elem, s)
		{
			...
			function initTouchSurface(){
				...
				if(testMsGesture){
					var gesture = new MSGesture();
					gesture.target = container[0];
				}

					container.addClass('scroll-no-touch-action');
				...
				container[0].addEventListener('MSPointerDown',onTouchStart,false);
					...
					function onTouchStart(e) {
						...
						container[0].addEventListener('MSPointerMove',onTouchMove,false);
						container[0].addEventListener('MSPointerUp',onTouchMove,false);
					}
					...
					function onTouchMove(ev) {
						...
						var touchPos = ev,
							dX = horizontalDragPosition, dY = verticalDragPosition,
							touchPosX = ev.clientX,
							touchPosY = ev.clientY;
						
						jsp.scrollTo(startX + touchStartX - touchPosX, startY + touchStartY - touchPosY);
						
						moved = moved || Math.abs(touchStartX - touchPosX) > 5 || Math.abs(touchStartY - touchPosY) > 5;
					...
					function onTouchEnd(){
						if(moved) {
							container[0].removeEventListener('MSPointerMove');
							container[0].removeEventListener('MSPointerUp');
							container.removeClass('scroll-no-touch-action');
							moved = false;
							return false;
						}
					}
