function updateURL(clearUrl) {
	...
	if (document.fForm.CONTENT_NAVIGATION_ID.options[0]) {
		cid = document.fForm.CONTENT_NAVIGATION_ID.options[0].value;
		aQuery.push("cid" + sEqual + cid);
		idTitle = cid;
		typeTitle = 'CONTENT';
	}
	 
	if (document.fForm.TPL_ID) {
		if (document.fForm.TPL_ID.value) {
			tpl = document.fForm.TPL_ID.value;
	...
	switch (tpl) {
		default:
		if (aQuery.length) {
			if (document.fForm.NAVIGATION_PARAMETERS.value) {
			...
			document.fForm.NAVIGATION_URL.value = sUrl + sJoin + getTitle(idTitle, typeTitle, clearUrl);
		}
		break;
	}
