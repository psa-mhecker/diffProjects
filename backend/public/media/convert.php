<?php
        ...
        if (!is_executable($wk)) {
            die('Unable to find the pdf converter');
        }
        if (!is_dir($tmpdir)) {
            die('tmpdir does not exists');
        }
        ...
        if ($tmp === false || !is_file($tmp)) {
            die('unable to create pdf temporary file');
        }

        $ret = exec("$wk $options $url $tmp 2>&1");
        if (preg_match('/done/i', $ret) < 1) {
            die('Unable to convert');
        }
        clearstatcache();

        if (!is_file($tmp)) {
            die('unable to find the generated pdf');
        }
