...
class Cms_Controller extends Pelican_Controller_Back
{
     ...
     * @var array
     */
    protected $decachePublicationOrchestra = [];
        ...
        if (Pelican_Db::$values[$this->workflowField."_ID"] && ! Pelican_Db::$values["isSubForm"]) {
            ...
            switch ($this->form_action) {
                ...
                case Pelican_Db::DATABASE_UPDATE:
                    {
                        if (Pelican_Db::$values[$this->workflowField."_DRAFT_VERSION"] == Pelican_Db::$values[$this->workflowField."_CURRENT_VERSION"]) {
                            ...
                            /*
                             * if (Pelican_Db::$values['STATE_ID'] == Pelican::$config["CORBEILLE_STATE"]) { Pelican_Db::$values[$this->workflowField . "_DRAFT_VERSION"] = $oConnection->queryItem("select max(" . $this->workflowField . "_VERSION) from #pref#_" . strtolower($this->workflowField) . "_version where " . $this->workflowField . "_ID=:ID AND LANGUE_ID=:LANGUE_ID", $aBind); } else {
                             */
                        ...
                        if (Pelican_Db::$values["STATE_ID"] == Pelican::$config["CORBEILLE_STATE"] && Pelican_Db::$values[$this->workflowField."_CURRENT_VERSION"] == NULL) {
                             Pelican_Db::$values["PUBLICATION"] =0;
                        }
                        if (Pelican_Db::$values["PUBLICATION"]) {
                            Pelican_Db::$values[$this->workflowField."_CURRENT_VERSION"] = Pelican_Db::$values[$this->workflowField."_DRAFT_VERSION"];
                            Pelican_Db::$values[$this->workflowField."_STATUS"] = 1;
    ...
    public  function clearOrchestraRouteCache() {
        $this->getContainer()->get('sonata.cache.symfony')->flush(array('routing'));
    }
     ...
     * @return __TYPE__
     */
    public static function setBatchPageOrder($page, $aId, $type, $langue = 1)
    {
        $oConnection = Pelican_Db::getInstance();
        $nb = count($aId);
     ...
     * @return __TYPE__
     */
    public static function cleanOrder($page)
    {
/*        Pelican_Cache::clean("Frontend/Page/Childall", $page, "", Pelican::$config["GROUP_DECACHE"]);
     ...
     * @return __TYPE__
     */
    public static function makeClearUrl($id, $type, $title = "", $path = "", $mediaId = "", $externalLink = "", $xiti = "")
    {
    ...
    protected function beforeCms()
    {
        ...
        if (isset($_POST['PAGE_ID']) && ($_POST['PAGE_ID'] != '') && isset($_POST['CONTENT_ID']) && ($_POST['CONTENT_ID'] == - 2)) {
            // Cas d'une création, on complète la liste des créateurs avec celle de la rubrique parente
    ...
    protected function beginForm(&$oForm)
    {
        ...
        if (valueExists($_SESSION[APP], "form_preview")) {
            ...
            if (! $_GET["uid"]) {
                $this->previewPage($_GET["id"], $baseurl);
            } else {
                $param = "cid";
                $this->getView()
                    ->getHead()
                    ->setScript("window.open('".$baseurl."/_/Index/preview?".$param."=".$_GET["id"]."', '_blank');");
            }
    ...
    protected function previewPage($pageId, $baseUrl)
    {
        ...
        $sql = 'SELECT
                        page_clear_url
                    FROM #pref#_page p
                    INNER JOIN #pref#_page_version pv ON p.langue_id = pv.langue_id AND p.page_id = pv.page_id AND p.page_draft_version= pv.page_version
                    WHERE
                    p.langue_id = :LANGUE_ID
                    AND p.site_id = :SITE_ID
                    AND p.page_id = :PAGE_ID
                    ';
        $clearUrl = $con->queryItem($sql,$bind);
            $this->getView()
                ->getHead()
                ->setScript("window.open('".$baseUrl.'/preview'.$clearUrl."','_blank');");
        ...
        if ($this->bVersioning && $this->values[$this->workflowField."_VERSION"] && ! $this->bPopup) {
            ...
            if ($versionResult) {
                ...
                foreach ($versionResult as $valeur) {
                    ...
                    if (! $this->readO) {
                        if ($i == 0 && !Psa_User_Backoffice::isProfileTranslator()) {
            ...
            if ($this->getEditModel()) {
                ...
                if ($this->workflowField) {
                    if ($this->bVersioning && $this->iVersionId) {
                         ...
                         */
                        if ($this->workflowField == "CONTENT" && ! $this->bPopup) { // PLA20130130 : correction du retour après dupliacation. TODO : vérifier si nécessaire en popup
                        ...
                        } elseif ($this->workflowField == "PAGE" && ! $this->bPopup) { // PLA201300201 : correction du retour après dupliacation. TODO : vérifier si nécessaire en popup
        ...
        if ($this->form_name == 'content' || $this->form_name == 'page') {
            if (! $this->values && $this->id != Pelican::$config["DATABASE_INSERT_ID"] && $this->lang) {
                ...
                if ($this->form_name == 'page') {
                    $page = true;
                    /*
                     * on récupère les valeurs de gabarit et de page parente
                     */
                    $sql = "select p.PAGE_ID, PAGE_PARENT_ID, TEMPLATE_PAGE_ID, PAGE_ORDER, PAGE_GENERAL, PAGE_PATH, PAGE_DISPLAY, PAGE_DISPLAY_NAV, PAGE_DISPLAY_SEARCH, PAGE_TITLE, PAGE_TITLE_BO from #pref#_page p inner join
                ...
                if (! $exists1) {
                    if ($this->form_name == 'page') {
                       ...
                       if ($site != $_SESSION[APP]['SITE_ID'] && $site !== []) { // si site est vide on est sur une page qui n'existe as dans cette langue NDP-3569
                ...
                if (! $exists2) {
                    $sql = "insert into #pref#_".strtolower($this->form_name)."_version
							...
							LANGUE_ID,".($page ? "TEMPLATE_PAGE_ID, ".$this->form_name."_DISPLAY,".$this->form_name."_DISPLAY_NAV,".$this->form_name."_DISPLAY_SEARCH," : "")."STATE_ID,
							".$this->form_name."_TITLE_BO,
							".$this->form_name."_TITLE
							) VALUES (
							:ID,
							:VERSION,
							:LANGUE_ID,".($page ? ":TEMPLATE_PAGE_ID,:PAGE_DISPLAY,:PAGE_DISPLAY_NAV,:PAGE_DISPLAY_SEARCH," : "").":STATE_ID,
							:TITLE_BO,
							:TITLE)";
        ...
        foreach ($aParentsIds as $iParentId) {
            foreach ($aPages as $aPage) {
                if ($aPage['PAGE_ID'] == $iParentId) {
                    $aPageLibPath[] = sprintf('%s|%s', $aPage['PAGE_ID'], $this->stripTags($aPage['PAGE_TITLE_BO']));
                }
            }
        }
        $sPageLibPath = implode('#', $aPageLibPath);
        $sPageLibPath = sprintf('%s#%s|[%s]', $sPageLibPath, $originalPage['PAGE_ID'], $this->stripTags($originalPage['PAGE_TITLE_BO']));
    ...
    private function stripTags($text) {
        ...
        return htmlentities(strip_tags(html_entity_decode($text)));
    }
    ...
    protected function addCmsPageParentByLanguage($pid, $langue_id)
    {
        if ($pid != '') {
            ...
            if (! $existsParent) {
                ...
                $sql = "select SITE_ID, PAGE_PARENT_ID, TEMPLATE_PAGE_ID, PAGE_ORDER, PAGE_PATH, PAGE_GENERAL, PAGE_DISPLAY, PAGE_DISPLAY_NAV, PAGE_DISPLAY_SEARCH, PAGE_TITLE, PAGE_TITLE_BO from #pref#_page p inner join
                                    #pref#_page_version pv on (p.PAGE_ID=pv.PAGE_ID AND p.PAGE_DRAFT_VERSION=pv.PAGE_VERSION AND p.LANGUE_ID=pv.LANGUE_ID)
                                    WHERE p.PAGE_ID=:ID";
     ...
     * @return array()
     */
    public static function getLanguageSite($lang = false)
    {
        $oConnection = Pelican_Db::getInstance();
    ...
    protected function execDecache()
    {
        ...
        if (Pelican_Db::$values["PUBLICATION"] || Pelican_Db::$values["DRAG_N_DROP"]) {
            ...
            if(!empty($this->decachePublicationOrchestra)){
                $this->listDecacheOrchestra = array_merge($this->listDecacheOrchestra, $this->decachePublicationOrchestra);            
            }
    ...
    protected function updateRewriting()
    {
        if ($this->bRewrite) {
            ...
            if (Pelican_Db::$values["REWRITE_ALIAS_URL"]) {
                ...
                Pelican_Db::$values["REWRITE_URL"]['200'] = array_map(array($this, "cleanRewriting"), Pelican_Db::$values["REWRITE_URL"]['200']);
            }
            ...
            if (Pelican_Db::$values["REWRITE_REDIRECT_URL"]) {
                ...
                Pelican_Db::$values["REWRITE_URL"]['301'] = array_map(array($this, "cleanRewriting"), Pelican_Db::$values["REWRITE_URL"]['301']);
            }
                    ...
                    foreach ($init["REWRITE_URL"] as $code => $rewrite) {
                        ...
                        foreach ($rewrite as $rewriting) {
                            if ($rewriting) {
                                ...
                                Pelican_Db::$values["CREATED_AT"] = date('Y-m-d');
        ...
        if ($_POST["form_action"] != Pelican::$config["DATABASE_DELETE"]) {
            ...
            if (isset($this->clearurlId) && Pelican_Db::$values[$field_url] == '') {
                if (Pelican_Db::$values[$field_id] != Pelican::$config["DATABASE_INSERT_ID"]) {
                    ...
                    Pelican_Db::$values[$field_url] = self::makeClearUrl(Pelican_Db::$values[$field_id], $this->clearurlId, Pelican_Text::unhtmlentities(Pelican_Db::$values[$field_title]), Pelican_Db::$values["PAGE_LIBPATH"], "");
...
It has been changed from state \"".t($aState[Pelican_Db::$values['old_STATE_ID']])."\" to state \"".t($aState[Pelican_Db::$values['STATE_ID']])."\" by user \"".$_SESSION[APP]['user']['name']."\" on ".date('d/m/Y')." at ".date('H\hi').".";
            ...
            $mail->setSubject(utf8_decode(Pelican::$config['SITE_NAME'].' "'.Pelican::$config['SITE']['INFOS']['SITE_LABEL'].'" information : Page "'.Pelican_Db::$values['PAGE_TITLE_BO'].'" changed to "'.t($aState[Pelican_Db::$values['STATE_ID']]).'"'));
            //$mail->send();
        }
    ...
    protected function extractMediaLinks($content)
    {
        ...
        $result = array_map(array($this,'extractMediaId'), $result);
    ...
    function cleanRewriting($url)
    {
        ...
        if ($url) {
            ...
            if (!isset($pathinfo["extension"])) {// la fin de l'url ne contient pas d'extension donc c'est un dossier
                $return = rtrim($return,'/')."/";
            }
        }
