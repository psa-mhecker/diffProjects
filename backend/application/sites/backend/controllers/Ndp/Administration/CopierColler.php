 ...
 *
 */
class Ndp_Administration_CopierColler_Controller extends Ndp_Controller
{
    ...
    public function editAction()
    {
        ...
        if (isset($_REQUEST['msg']) && !empty($_REQUEST['msg'])) {
            ...
            $form .= $oForm->createComboFromSql($oConnection, 'SITE_ID_SOURCE', t('PAYS_SOURCE'), $sSqlListe, $sSqlListe, true, $readO, "1", false, "", true, false, "onchange=\"callAjax('/Ndp_Administration_CopierColler/ajaxShowComboLang',this.value);\"");
                ...
                while ($i < Pelican_Db::$values['NB_LANGUES']) {
                    if (Pelican_Db::$values['LANGUE_SOURCE_'.$i] && Pelican_Db::$values['LANGUE_CIBLE_'.$i]) {
                        ...
                        if ($pageComposite->verifGabaritUnique($pageIdSource['PAGE_ID'], $langIdSource, $langIdCible, $siteIdCible)) {
                            $msg = t('GABARIT_ONE_USE_ALREADY_EXIST');
                            debug('eee');
    ...
    public function getHtmlComboLang($idSiteSource)
    {
        ...
        $return = $oFormAjax->createRadioFromList("LANGUE_SOURCE", t('LANGUE_SOURCE'), $aLanguesSource, null, true, $readO, "h", false, "onchange=\"callAjax('/Ndp_Administration_CopierColler/ajaxShowArborescence',this.value, $idSiteSource);\"");
        $oConnection->close();
