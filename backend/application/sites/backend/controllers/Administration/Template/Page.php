...
class Administration_Template_Page_Controller extends Pelican_Controller_Back
{
    ...
    protected function _zoneTemplateSave()
    {
        ...
        if (Pelican_Db::$values["id"] && Pelican_Db::$values["form_action"] == Pelican_Db::DATABASE_DELETE) {
            ...
            foreach($aChild as $table) {
                $oConnection->query("delete from #pref#_".$table." where ZONE_TEMPLATE_ID=:ID", array(
    ...
    public function zonetemplateAction()
    {
        ...
        if ($this->form_action == "INS") {
            $this->values["ZONE_CACHE_TIME"] = -2;
        }

        $oForm->createInput("ZONE_CACHE_TIME", t('NDP_CACHE'), 5, "", true, $this->values["ZONE_CACHE_TIME"], $this->readO, 5);
        $oForm->createLabel(t('NDP_CACHE_SPECIAL_VALUES').':', ''); //valeurs spéciales du cache
        $oForm->createLabel(' 0 :', 'NDP_NO_CACHE');
        $oForm->createLabel('-1 :', 'NDP_MAXIMUM_CACHE');
        $oForm->createLabel('-2 :', 'NDP_DEFAULT_CACHE');//valeur par defaut du cache définit dans les paramètres globaux du projet
        $this->endForm($oForm);
        endFormTable();
        echo "<p align=\"center\">";
