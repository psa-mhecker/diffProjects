        ...
        if ($_GET["action"] === "0" || ! empty($_POST)) {
            ...
            if ($oPlugin) {
                ...
                if (($plugin["plugin_module"])) {
                    ...
                    $aSelected = "";
                    $form .= $this->oForm->_getValuesFromSql($strSQL, $aData);
                    if ($plugin["plugin_navigation"]) {
                ...
                if ($plugin["plugin_navigation"] || $plugin["plugin_module"] || $plugin["plugin_content"]) {
                    ...
                    $form .= $this->oForm->createAssocFromSql($oConnection, "PROFILE_ID", "Profil(s) associés", "", "", true, true, $this->readO, 12, 250, false, array(
                        "site",
                        $comboQuery,
                        $searchQuery,
                    ));
