...
class Index_Controller extends Pelican_Controller_Back
{
    ...
    public function indexAction()
    {
        ...
        $this->assign('left_middle', Backoffice_Div_Helper::leftMiddle($this->aHierarchie["onglet"], $_GET["view"]),
            false);
    ...
    public function childAction()
    {
        ...
        if (!empty($flash)) {
            $this->assign('flash_message', $flash);
        }
    ...
    public function loginAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (isset($_POST["login"]) && isset($_POST["password"])) {
                ...
                $oUserPsa = Pelican_Factory::newInstance('User.Backoffice', $sLogin, $sPwd);

                /**
                 * Connexion Ldap
                 */
                $userLdap = new Psa_Dsin_User($sLogin, $sPwd);

                $bIsLdap = false;
                /* TODO GESTION DES CAS D'ERREUR */
                // SI ON RENTRE PAS C EST QU ON UTILISE LE COMPTE ADMIN
                if ($sLogin != Pelican::$config["LOGIN_ADMIN_BO"] && $sPwd != md5(Pelican::$config["PWD_ADMIN_BO"])) {
                    ...
                    if ($bLdapLogged) {
                        ...
                        if ($aStrongestRights['%%'] == 'ADMINISTRATEUR') {
                            // cas ADMINISTRATEUR + TOUS LES PAYS
                            $bAdmin = true;
                        }
                        $oUserPsa->deleteUser($sLogin);
                        ...
                        $oUserPsa->createUser($aParams);
                    }
                }

                $oUserPsa->login($sLogin, $sPwd, $bIsLdap);
                $log = false;
                $choose = false;

                if ($oUserPsa->isLoggedIn()) {
                    $result = $oUserPsa->getFullInfos();
                    /*
        ...
        $this->assign('load',
            "onload=\"if (document.location.href!=parent.location.href) top.location.href='".Pelican::$config["INDEX_PATH"]."/_/Index/login';document.fLogin.".$focusedInput.".focus();\"");
        ...
        if (! isset($_SESSION[APP]["langComboSite"])) {
            /*
             * Combo de langue du BO
             */
            $_SESSION[APP]["langComboSite"] = '';
            $BO['INFOS'] = Pelican_Cache::fetch("Frontend/Site", 1);
            $LangRef = Pelican_Cache::fetch("Language");
            if (! empty($BO['INFOS']["LANG"])) {
                foreach ($BO['INFOS']["LANG"] as $langid) {
                    $aLangCombo[] = Pelican_Html::nobr(Pelican_Html::input(array(
                        'type' => 'radio',
                        'name' => 'lang',
                        'id' => 'lang'.$langid,
                        'value' => $langid,
                        'checked' => ($langid == $_SESSION[APP]['LANG']?"checked":""),
                        'onchange' => "change_lang(this);"
                    ), Pelican_Html::img(array('src' => Pelican::$config["LIB_PATH"]."/Pelican/Translate/public/images/flags/".strtolower($LangRef[$langid]['LANGUE_CODE']).".png"))."&nbsp;".$LangRef[$langid]['LANGUE_TRANSLATE']."&nbsp;"."&nbsp;"));
                }
                $langComboSite =  implode("", $aLangCombo);
                $_SESSION[APP]["langComboSite"] = $langComboSite;
            }
        }
        if ($_SESSION[APP]["langComboSite"]) {
            $this->assign('langCombo', $_SESSION[APP]["langComboSite"], false);
        } else {
            $this->assign('langCombo', '');
        }
    ...
    protected function _indexCommon()
    {
        ...
        $head->setScript("var libDir='".Pelican::$config["LIB_PATH"]."';");
        $head->setScript("var mediaDir='".(valueExists($_GET,
                "popup_content") ? "." : Pelican::$config["MEDIA_LIB_PATH"])."';");
