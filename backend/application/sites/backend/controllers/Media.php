...
class Media_Controller extends Pelican_Controller_Back
{
     ...
     * @var mixed
     */
    protected $defaultOrder = 'MEDIA_PATH';
    ...
    public function init()
    {
        ...
        if (valueExists($_GET, 'delForcage')) {
            ...
            $file = getUploadRoot(
                Pelican_Media::getFileNameMediaFormat(
                    Pelican_Media::getMediaPath($_GET['preview']),
                    $_GET['delForcage']
                )
            );
            if (file_exists($file)) {
        ...
        $this->aAllowedExtensions = Pelican_Media::getAllowedExtensions();
    }
    ...
    public function editAction()
    {
        ...
        if (!($_REQUEST['view'] == 'streamlike' && $_REQUEST['media'] == '')) { // ni INS ni DEL pour streamlike
    ...
    public function saveAction()
    {
        if ($_REQUEST['type'] == 'folder') {
            ...
            } elseif ($_REQUEST['type'] == 'streamlike' || $_REQUEST['view'] == 'streamlike') {
                // pas de traitement streamlike
            } else {
                /********************************
    ...
    public function popupAction()
    {
        ...
        $head->setScript(
            "var vOnline = '".(isset(Pelican::$config['ACTION_ONLINE']) ? Pelican::$config['ACTION_ONLINE'] : '')."';"
        );
        $head->setScript("var libDir='".Pelican::$config['LIB_PATH']."';");
...
window.close();
        ...
        if ($tiny) {
            $head->endJs(Pelican::$config['LIB_PATH'].Pelican::$config['LIB_OTHER'].'/tiny_mce/tinymce.min.js');
            $head->endJs(Pelican::$config['LIB_PATH'].'/External/tiny_mce/plugins/betd_patch/js/betd_patch.js');
            $head->endJs(
                Pelican::$config['LIB_PATH'].Pelican::$config['LIB_OTHER'].'/tiny_mce/plugins/betd_media/js/betd_media.js'
            );
        }
        ...
        $this->assign(
            'left_middle',
            Backoffice_Div_Helper::leftMiddle($this->aHierarchie['onglet'], $_GET['view'], 'popup'),
            false
        );
    ...
    protected function _getFilter()
    {
        ...
        if ($zone == 'popup' && !empty($type)) {
            ...
            foreach ($this->aAllowedExtensions as $key => $value) {
                if ($type == $key || ($type == 'video' && ($key == 'youtube' || $key == 'streamlike'))) {
                    $oTab->addTab(
                        $value['libelle'],
                        'onglet_filter_'.$key,
                        ($type == $key),
                        '',
                        "parent.setFilter('".$key."')",
                        '',
                        'petit'
                    );
                }
    ...
    protected function _saveFolder()
    {
        ...
        switch ($_REQUEST['action']) {
            case 'save': {
                if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                    $this->_folder = $oConnection->queryItem(
                        'select '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' FROM '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' WHERE '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$_REQUEST['root']
                    );
                    $path = implode(
                        ' > ',
                        array(
                    ...
                    $aBind[':SITE_ID'] = $_SESSION[APP]['SITE_ID'];
                    $nb = $oConnection->queryItem(
                        'select count(*)-1 from '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' T1 where T1.'.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' = :PATH AND T1.SITE_ID = :SITE_ID',
                        $aBind
                    );
                    if (((int) $nb == -1)) {
                        ...
                        $this->_jsForm = "alert('".str_replace("'", "\'", t('EXIST_CAT', 'js'))."');\r\n";
                    }
                } else {
            ...
            case 'update': {
                ...
                if (Pelican_Db::$values['MEDIA_DEBUT_DATE']) {
                    $bind = $oConnection->setBindValue(
                        $aBind,
                        $indexBind++,
                        Pelican_Db::$values['MEDIA_DEBUT_DATE'],
                        'date',
                        false
                    );
                ...
                if (Pelican_Db::$values['MEDIA_FIN_DATE']) {
                    $bind = $oConnection->setBindValue(
                        $aBind,
                        $indexBind++,
                        Pelican_Db::$values['MEDIA_FIN_DATE'],
                        'date',
                        false
                    );
                ...
                if (Pelican_Db::$values['MEDIA_CREDIT']) {
                    $bind = $oConnection->setBindValue(
                        $aBind,
                        $indexBind++,
                        Pelican_Db::$values['MEDIA_CREDIT'],
                        'string'
                    );
                ...
                if ($fieldUpdate) {
                    $bind = $oConnection->setBindValue(
                        $aBind,
                        $indexBind++,
                        Pelican_Db::$values[Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH']],
                        'string'
                    );
                    $sql = 'update #pref#_media set '.implode(
                            ',',
                            $fieldUpdate
                        ).' where '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].' in (SELECT '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].' from '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' where ('.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' like '.$oConnection->getConcatClause(
                            array(
                                $bind,
                                "'%'",
                            )
                    ...
                    if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                        ...
                        if (((int) $countFolder >= 0)) {
                            ...
                            if (!$countDir) {
                                ...
                                $this->_jsForm .= "alert('".str_replace(
                                        "'",
                                        "''",
                                        t('POPUP_MEDIA_MSG_EMPTY_FOLDER')
                                    )."');";
                            }
                        }
                    } else {
            ...
            case 'move': {
                $oConnection->query(
                    'update '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' set '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PARENT'].'='.$_REQUEST['to'].' where '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$_REQUEST['from']
                );

                /*
                 * Mise à jour des chemins
                 */
                $old = $oConnection->queryItem(
                    'select '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' from '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' where '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$_REQUEST['from']
                );
                $new0 = $oConnection->queryItem(
                    'select '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' from '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' where '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$_REQUEST['to']
                );
                $path0 = explode(' > ', $old);
                $tmp = array_pop($path0);
                $new = $new0.' > '.$tmp;
    ...
    protected function _saveFile()
    {
        ...
        switch ($action) {
            case 'save': {
                ...
                if ($_FILES['file_name']['name']) {
                    ...
                    if ($file_name['tmp_name'] == 'none') {
                        ...
                        $_REQUEST['file_name'] = (is_string($_REQUEST['file_name'])) ? strtr(
                            $_REQUEST['file_name'],
                            '????????????????????????????????????????????????????',
                ...
                $messageSize = $this->checkMaxSize($_REQUEST['view'], $_REQUEST['size'], $_REQUEST['file_name']);
                if (!empty($messageSize)) {
                    $this->_jsForm = "alert('".str_replace("'", "\'", $messageSize)."');\r\n";
                ...
                if (!$this->aAllowedExtensions[$_REQUEST['view']][strtolower($pathinfo['extension'])]) {
                    ...
                    if (is_array($aExtensionValid)) {
                        $ExtensionValidMessage = t('NDP_ERROR_FILE_UPLOAD_FORMAT').'\r\n';
                        $ExtensionValidMessage .= t('NDP_ONLY_FILE_WITH_FORMAT').' ';
                        $ExtensionValidMessage .= implode(', ', array_keys($aExtensionValid));
                        $ExtensionValidMessage .= ' '.t('NDP_ARE_ACCEPTED');
                        $this->_jsForm = "alert('".str_replace("'", "\'", $ExtensionValidMessage)."');\r\n";
                    }
                    $this->_jsForm .= "history.go(-1);\r\n";
                    break;
                }
                ...
                $upfile = getUploadRoot(Ndp_Media::fileName($filename, $ID));
                ...
                if (!$this->aAllowedExtensions[$_REQUEST['view']][strtolower(
                        $pathinfo['extension']
                    )] && $_REQUEST['view'] != 'all' && ($_REQUEST['view'] == 'video' && $_REQUEST['MEDIA_ID_REFERENT'] == '')
                ) {
                    // L'extension n'est pas autorisée
                    $this->_jsForm = "alert('".str_replace("'", "\\'", t('POPUP_UPLOAD_WRONG_TYPE'))."');\r\n";
                    $this->_jsForm .= "document.location.href='".$_SERVER['HTTP_REFERER']."';\r\n";
                    // } elseif ($this->_jsForm) {
                    // $this->_jsForm .= "document.location.href='" . $_SERVER["HTTP_REFERER"] . "';\r\n";
                    ...
                    $this->_jsForm = "alert('".str_replace(
                            "'",
                            "\\'",
                            t('POPUP_UPLOAD_ALREADY_EXISTS')
                        ).' '.str_replace("'", "\\'", $database_exists['path'])."');\r\n";
                    $this->_jsForm .= ''.JSTARGET.".previewMedia('".rawurlencode(
                            $database_exists['file']
                        )."', ".$database_exists['id'].");\r\n";
                } else {
                    ...
                    if ($_REQUEST['view'] == 'video' && $_REQUEST['MEDIA_ID_REFERENT'] != '' && !is_string($tmp_name)) {
                        if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                            ...
                            $oConnection->updateTable(
                                Pelican_Db::$values['form_action'],
                        ...
                        if ($_REQUEST[Pelican::$config['FW_MEDIA_FIELD_PATH']]) {
                            @unlink(
                                Pelican_Media::cleanDirectory(
                                    Pelican::$config['MEDIA_ROOT'].$_REQUEST[Pelican::$config['FW_MEDIA_FIELD_PATH']]
                                )
                            );
                        }
                        ...
                        if (!copy($tmp_name, $upfile)) {
                            ...
                            $this->_jsForm = "alert('".str_replace("'", "\'", $message)."');\r\n";
                            $this->_jsForm .= "history.go(-1);\r\n";
                        } else {
                            ...
                            $info = Pelican_Media::cleanDirectory(
                                '/'.str_replace(Pelican::$config['MEDIA_ROOT'], '', $file)
                            );
                            if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                                Pelican_Db::$values = $_REQUEST;
                                ...
                                if (Pelican::$config['FW_MEDIA_FIELD']['TITLE'] && !Pelican_Db::$values[Pelican::$config['FW_MEDIA_FIELD']['TITLE']]) {
                                    Pelican_Db::$values[Pelican::$config['FW_MEDIA_FIELD']['TITLE']] = str_replace(
                                        $pathinfo['extension'],
                                        '',
                                        basename($info)
                                    );
                            ...
                            if ($_REQUEST['zone'] == 'upload') {
                                ...
                                if ($this->aAllowedExtensions['image'][strtolower($pathinfo['extension'])]) {
                                    ...
                                    Pelican_Media::reduceSize(
                                        $imageinfo,
                                        (Pelican::$config['IMG_WIDTH_THUMBNAIL'] ? Pelican::$config['IMG_WIDTH_THUMBNAIL'] : '50')
                                    );
                                    $thumbnailTag = '<img src="'.$info.'" alt="'.basename(
                                            $info
                                        ).'" '.$imageinfo[3].' border="0">';
                                } else {
                                    $thumbnailTag = basename($info);
                                }
                                $this->_jsForm = ''.JSTARGET.".setMedia('".rawurlencode($info)."','".rawurlencode(
                                        $fileTag
                                    )."','".rawurlencode($thumbnailTag)."','".$id."');\r\n";
                            }
                            $this->_jsForm .= JSTARGET.'.goBack();';
                        }
                        // CACHE
                        $pathCache = Pelican_Media::cleanDirectory(
                            '/'.str_replace(Pelican::$config['MEDIA_ROOT'], '', $file)
                        );
                    }
                }
            ...
            case 'del': {
                ...
                $usage = Ndp_Media::checkMediaUsageDetail($_REQUEST['id']);

                if ($_REQUEST['id'] && !$usage[0] && !$usage[1] && !$usage[2]) {
                    // suppression du fichier
                    ...
                    if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                        ...
                        $oConnection->updateTable(
                            Pelican::$config['DATABASE_DELETE'],
                            '#pref#_media_format_intercept',
                            'MEDIA_FORMAT_ID'
                        );
                     ...
                     */
                    $this->_jsForm = "alert('".str_replace("'", "\'", t('SUPP_IMP_USE', 'js'))."');\r\n";
                }
                break;
            }
            case 'move': {
                $oConnection->query(
                    'update '.Pelican::$config['FW_MEDIA_TABLE_NAME'].' set '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$_REQUEST['to'].' where '.Pelican::$config['FW_MEDIA_FIELD_ID'].'='.$_REQUEST['from']
                );
                $this->_jsForm = JSTARGET.'.goBack();';
                break;
            }
            default: {
                $message = t('POPUP_UPLOAD_TOO_BIG_PHP').' '.ini_get('post_max_size').'.';
                $this->_jsForm = "alert('".str_replace("'", "\'", $message)."');\r\n";
                $this->_jsForm .= "history.go(-1);\r\n";
                break;
            }
        ...
        // Sauvegarde de titre et alt en version i18n
        if (isset(Pelican_Db::$values['MEDIA_ID']) && Pelican_Db::$values['MEDIA_ID'] != -2) {
            $params = $this->getParams();
            $params['MEDIA_ID'] = Pelican_Db::$values['MEDIA_ID'];
            $params['HMVC'] = true;
            Pelican_Request::call('_/Media_AltTranslation/save', $params);
        }
    }
    ...
    protected function _makePreview($values = array(), $prop = true)
    {
        ...
        switch ($type) {
            case 'image': {
                ...
                if ($_GET['format'] && $_REQUEST['format'] != 'no') {
                    $fileTag = '<img id="imgMediaFormat" owidth="'.$sizeOrigine[0].'" oheight="'.$sizeOrigine[1].'" hspace="5" vspace="5" border="1" src="'.Pelican::$config['MEDIA_LIB_PATH'].'/image_format.php?path='.rawurlencode(
                            $file
                        ).'&format='.$_GET['format'].'" style="border-color:#CACACA;" />';
                } else {
            ...
            case 'streamlike': {
                // Tag de prévisualisation
                $fileTag = '<iframe width="230" height="157" src="'.$values[Pelican::$config['FW_MEDIA_FIELD_PATH']].'" frameborder="0" allowfullscreen></iframe><br /><br />';
                break;
            }
            default: {
                // Tag de prévisualisation
                $fileTag = '<img src="'.Pelican::$config['LIB_PATH'].Pelican::$config['LIB_FILE'].'/images/'.strtolower(
                        $pathinfo['extension']
                    ).'.gif" alt="" border="0" align"middle">';
                $fileTag .= '&nbsp;<a href="'.Pelican::$config['MEDIA_HTTP'].$file.'" target="_blank"/>'.$pathinfo['basename'].'</a>';
                if ($type == 'video') {
                    $fileTag .= '&nbsp;'.Pelican_Html::img(
                            array(
        ...
        $complement = '<tr><td class="formlib">'.t(
                'POPUP_MEDIA_LABEL_LAST_ACCES'
            ).'&nbsp;:</td><td  class="formval">'.$this->_fileMtimeMedia($file, $values).'</td></tr>';
        // partie supérieure : prévisualisation
        if ($target) {
            $preview = '<img src="'.Pelican::$config['LIB_PATH'].'/public/images/pixel.gif" width="15" height="1" alt="" border="0">'.$fileTag.'<a href="'.Pelican::$config['MEDIA_HTTP'].$file.'" target="_blank"><img id="pictoView" src="'.Pelican::$config['MEDIA_LIB_PATH'].'/images/view.gif" width="15" height="15" alt="'.t(
                    'POPUP_MEDIA_MSG_REAL_SIZE'
        ...
        if ($_REQUEST['type'] == 'image' && $_REQUEST['format'] != 'no') {
            ...
            $preview = '<center>'.$preview.'<br />'.$this->_makeMediaFormat(
                    $file,
                    $values[Pelican::$config['FW_MEDIA_FIELD_LIB']],
                    $previewSize
                ).'</center>';
        } else {
        ...
        if ($values['FORCAGE'] && $_REQUEST['format'] != 'no') {
            foreach ($values['FORCAGE'] as $force) {
                ...
                $forcage .= '&nbsp; <img src="'.Pelican::$config['MEDIA_LIB_PATH'].'/images/view.gif" alt="'.t(
                        'SHOW_FORMAT'
                    ).'" border="0" hspace="3" align="middle" onclick="viewMediaFormat('.$force['MEDIA_FORMAT_ID'].')" style="cursor:pointer"/>';
                if ($prop) {
                    $forcage .= '<img src="'.Pelican::$config['MEDIA_LIB_PATH'].'/images/media_del.gif" alt="'.t(
                            'ALT_DELETE_FORCAGE'
                        ).'" border="0" hspace="3" align="middle" onclick="delForcage('.$force['MEDIA_FORMAT_ID'].')" style="cursor:pointer"/><br />';
                }
            }
            $preview .= '<tr><td class="formlib">'.t(
                    'FORCED_FORMAT'
                ).'</td><td class="formval">'.$forcage.'</td></tr>';
        }
        /*
         * CHEMIN ET URL
         */
        if ($_REQUEST['type'] != 'youtube' && $_REQUEST['type'] != 'streamlike') {
            ...
            if ($this->_updateList) {
                ...
                if ($this->_updateList['date']) {
                    $where[] = Pelican::$config['FW_MEDIA_FIELD_CREATION_DATE'].' = '.$oConnection->dateStringToSql(
                            $this->_updateList['date']
                        );
                }
                if (!$values[Pelican::$config['FW_MEDIA_FIELD_MD5']] && $values[Pelican::$config['FW_MEDIA_FIELD_PATH']]) {
                    $where[] = Pelican::$config['FW_MEDIA_FIELD_MD5'].'='."'".md5_file(
                            Pelican_Media::cleanDirectory(
                                Pelican::$config['MEDIA_ROOT'].$values[Pelican::$config['FW_MEDIA_FIELD_PATH']]
                            )
                        )."'";
                }
                if ($where) {
                    $strSQL = 'update '.Pelican::$config['FW_MEDIA_TABLE_NAME'].' set '.implode(
                            ',',
                            $where
                        ).' where '.Pelican::$config['FW_MEDIA_FIELD_ID'].'='.$_REQUEST['id'];
                    $oConnection->query($strSQL);
                }
            }
            $taille = formatSize($this->_fileSizeMedia($file, $values));
        } elseif ($_REQUEST['type'] == 'youtube') {
        ...
        } elseif ($_REQUEST['type'] == 'streamlike') {
            $application = t('MEDIA_TYPE_STREAMLIKE');
            $format = t('MEDIA_RATIO').' '.$values['FW_MEDIA_FIELD_RATIO'];
            $taille = $values[Pelican::$config['FW_MEDIA_FIELD_WEIGHT']].' s.';
        }
        if ($taille) {
     ...
     * @return mixed
     */
    protected function _makeForm()
    {
        $oConnection = Pelican_Db::getInstance();
        $message = '';
        if ($_REQUEST['root'] && $_REQUEST['type'] != 'streamlike') {
            if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                /*
                 * MODE BDD
                 */
                $this->_folder = $oConnection->queryItem(
                    'select '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' as "folder" FROM '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' WHERE '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$_REQUEST['root']
                );
            } else {
                /*
                 * MODE PHYSIQUE
                 */
                $_REQUEST['root'] = Pelican::$config['MEDIA_ROOT'].str_replace(
                        Pelican::$config['MEDIA_ROOT'],
                        '',
                        $_REQUEST['root']
                    );
        ...
        switch ($_REQUEST['type']) {
            ...
            default: {
                ...
                if ($_REQUEST['action'] == 'edit' || !$_REQUEST['action']) {
                    if ($_REQUEST['type'] == 'streamlike') {
                        $configForm['submit'] = ''; // on ne doit pas avoir de bouton submit
                        $values = $this->getStreamlikeValues();
                    } elseif (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                        // recherche du média dans la base de donnée
                        if ($_REQUEST['id']) {
                            $values = $this->getValuesFromDb();
                        }
                    } else {
                        $configForm['submit'] = ''; // on ne doit pas avoir de bouton submit
                    }
        ...
        if ($_REQUEST['type'] == 'video') {
            $aVideo = $oConnection->queryTab(
                'SELECT MEDIA_PATH FROM #pref#_media WHERE MEDIA_ID_REFERENT = :ID',
                array(
                    ':ID' => $this->id,
                )
            );
        ...
        */
        ...
        if ($configForm['label'] && !$readO) {
            if ($_REQUEST['view'] == 'youtube') {
                ...
                if ($configForm['action'] == 'save') {
                    $form .= $oForm->createBrowse(
                        $configForm['name'],
                        $configForm['label'].' ('.t('MAX').' '.ini_get('upload_max_filesize').')',
                        '',
                        $configForm['required'],
                        (isset($_FILES['file_name']) ? $_FILES['file_name'] : ''),
                        $readO,
                        25,
                        false,
                        ''
                ...
                $form .= $oForm->createComboFromList(
                    'YOUTUBE_PRIVACYSTATUS',
                    t('YOUTUBE_PRIVACYSTATUS'),
                    $aPrivacyStatus,
                    $privacyStatus,
                    false,
                    $readO,
                ...
                $form .= $oForm->createTextArea(
                    'YOUTUBE_RECORD_LOC_DESC',
                    t('YOUTUBE_RECORD_LOC_DESC'),
                    false,
                    $values['YOUTUBE_RECORDINGDETAILS']['locationDescription'],
                    200
                );
                $form .= $oForm->createInput(
                    'YOUTUBE_RECORD_LOC_LAT',
                    t('YOUTUBE_RECORD_LOC_LAT'),
                    10,
                    'float',
                    false,
                    $values['YOUTUBE_RECORDINGDETAILS']['location']['latitude'],
                    $readO,
                    10
                );
                $form .= $oForm->createInput(
                    'YOUTUBE_RECORD_LOC_LNG',
                    t('YOUTUBE_RECORD_LOC_LNG'),
                    10,
                    'float',
                    false,
                    $values['YOUTUBE_RECORDINGDETAILS']['location']['longitude'],
                    $readO,
                    10
                );
                ...
                if ($configForm['type'] == 'file') {
                    $form .= $oForm->createBrowse(
                        $configForm['name'],
                        $configForm['label'].' ('.t('MAX').' '.Pelican::$config['MAX_SIZE'][strtoupper(
                            $_REQUEST['view']
                        )]['Label'].')',
                        '',
                        $configForm['required'],
        ...
        if ($_REQUEST['view'] == 'streamlike') {
            $form .= $oForm->createLabel(t('DATE_UPDATE'), $values['FW_MEDIA_FIELD_LAST_UPDATE']);
            $form .= $oForm->createLabel(
                t('MEDIA_CREDITS'),
                $values[Pelican::$config['FW_MEDIA_FIELD']['MEDIA_CREDIT']]
            );
            $form .= $oForm->createLabel(t('MEDIA_VISIBILITY'), $values['FW_MEDIA_FIELD_VISIBILITY']);
        }
        ...
        if (!$showForm) {
            $form .= $oForm->createHidden(
                'folder_name_sauve',
                $this->_folder_properties[Pelican::$config['FW_MEDIA_FIELD_FOLDER_NAME']]
            );
        }
        ...
        if ($_REQUEST['view'] != 'streamlike') {
            ...
            if ($_SESSION[APP]['user']['main'] && $_REQUEST['type'] != 'folder') {
                ...
                $form .= $oForm->createComboFromList(
                    ...
                    t('Createur'),
                    $aUsers,
                    $values['MEDIA_CREATION_USER'],
                    false,
                    $readO,
                    '1'
                );
                // formulaire titre/alt multilangue
                $params = $this->getParams();
                $params['MEDIA_ID'] = $this->id;
                $params['oForm'] = &$oForm;
                $form .= Pelican_Request::call('_/Media_AltTranslation/', $params);
            } else {
        ...
        $form .= $oForm->createHidden(
            'MEDIA_CREATION_DATE',
            (!$values['MEDIA_CREATION_DATE'] ? ':DATE_COURANTE' : $values['MEDIA_CREATION_DATE'])
        );
        if (!$readO) {
            $form .= $oForm->createFreeHtml('<tr>');
            $form .= $oForm->createFreeHtml('<td class="formval" colspan="2" align="center">');
            if ($_REQUEST['tmp_name']) {
                $form .= $oForm->createButton(
                    'replace',
                    t('POPUP_MEDIA_LABEL_REPLACE'),
                    'document.fForm.replace.value=true;document.fForm.submit();'
                );
        ...
        if (Pelican::$config['FW_MEDIA_FIELD_ALT']) {
            $preview['js'] .= JSTARGET.".current.fileAttribut[\"alt\"]='".str_replace(
                    "'",
                    "\\'",
                    $values[Pelican::$config['FW_MEDIA_FIELD_ALT']]
                )."';\r\n";
        }
        $form .= '<script type="text/javascript">';
        $form .= JSTARGET.'.current.usage = '.(($usage[0] || $usage[1] || $usage[2]) ? 'true' : ($prop ? 'false' : 'true')).';';
        $form .= JSTARGET.".setMedia('".rawurlencode($preview['file'])."','".rawurlencode(
                $preview['fileTag']
            ).">','".rawurlencode($preview['thumbnailTag'])."','".$id."');";
    ...
    protected function isOwner(array $values)
    {
        $return = true;
        if (!empty($values)) {
            // check is we are ownser
            $return = (($values['MEDIA_CREATION_USER'] == $_SESSION[APP]['user']['id']) || $_SESSION[APP]['user']['main']);

            // check if we are on the same site
            $return = $return && ($values['SITE_ID'] == $_SESSION[APP]['SITE_ID']);

        }
        // dans le cas de streamlike on est forcément en lecture seul
        if ($_REQUEST['type'] == 'streamlike') {
            $return = false;
        }
    ...
    protected function getStreamlikeValues()
    {
        ...
        $mediaStreamlikeId = $this->_streamlikeExist($details['id'], $details['path']);

        $_REQUEST['id'] = $mediaStreamlikeId;
        $values['id'] = $_REQUEST['id'];
        $values['preview'] = $_REQUEST['preview'];
        $values[Pelican::$config['FW_MEDIA_FIELD_ID']] = $mediaStreamlikeId;
        $values[Pelican::$config['FW_MEDIA_FIELD_LIB']] = $details['title'];
        $values[Pelican::$config['FW_MEDIA_FIELD_FILE']] = $details['title'];
        $values[Pelican::$config['FW_MEDIA_FIELD_PATH']] = $details['url'];
        $values[Pelican::$config['FW_MEDIA_FIELD_TYPE']] = 'streamlike';
        $values[Pelican::$config['FW_MEDIA_FIELD_CREATION_DATE']] = $details['creation_date'];
        $values['MEDIA_COMMENT'] = $details['description'];
        $values[Pelican::$config['FW_MEDIA_FIELD']['MEDIA_CREDIT']] = $details['credits'];
        $values['FW_MEDIA_FIELD_LAST_UPDATE'] = $details['lastupdated_date'];
        $values[Pelican::$config['FW_MEDIA_FIELD_WEIGHT']] = $details['time'];
        $values['FW_MEDIA_FIELD_RATIO'] = $details['ratio'];
        $values['FW_MEDIA_FIELD_VISIBILITY'] = $details['visibility'];

        return $values;
    }

    protected function getValuesFromDb()
    {
        ...
        if (Pelican::$config["FW_MEDIA_FIELD_CREATION_DATE"]) {
            $sql .= ", ".$connection->dateSqlToString(Pelican::$config["FW_MEDIA_FIELD_CREATION_DATE"])." as \"".Pelican::$config["FW_MEDIA_FIELD_CREATION_DATE"]."\"";
        }
        if (Pelican::$config["FW_MEDIA_FIELD_DEBUT_DATE"]) {
            $sql .= ", ".$connection->dateSqlToString(Pelican::$config["FW_MEDIA_FIELD_DEBUT_DATE"])." as \"".Pelican::$config["FW_MEDIA_FIELD_DEBUT_DATE"]."\"";
        }
        if (Pelican::$config["FW_MEDIA_FIELD_FIN_DATE"]) {
            $sql .= ", ".$connection->dateSqlToString(Pelican::$config["FW_MEDIA_FIELD_FIN_DATE"])." as \"".Pelican::$config["FW_MEDIA_FIELD_FIN_DATE"]."\"";
        }
        if (Pelican::$config["FW_MEDIA_FIELD_EXPIRATION_DATE"]) {
            $sql .= ", ".$connection->dateSqlToString(Pelican::$config["FW_MEDIA_FIELD_EXPIRATION_DATE"])." as \"".Pelican::$config["FW_MEDIA_FIELD_EXPIRATION_DATE"]."\"";
        }
        ...
        return $connection->queryForm($sql);
    }

    protected function _streamlikeExist($id, $path)
    {
        $oConnection = Pelican_Db::getInstance();
        $aBind = array(
            ':MEDIA_REMOTE_ID' => $oConnection->strToBind($id),
        );
        $iMediaId = $oConnection->queryItem(
            'SELECT media_id FROM #pref#_media WHERE MEDIA_REMOTE_ID = :MEDIA_REMOTE_ID',
            $aBind
        );
    ...
    protected function getStreamlikeList()
    {
        /*
        * gestion des videos Streamlike
        */
        $streamlikeConfig = Service_Streamlike::getStreamlikeConfig($_SESSION[APP]['SITE_ID']);
        $cachetime = Service_Streamlike::getStreamlikeCachetime($streamlikeConfig['STREAMLIKE_CACHETIME']);

        if ($_REQUEST['by_streamlike_id'] != '') {
            $rs = Pelican_Cache::fetch(
                'Service/Streamlike',
                array(
                    'id',
                    null,
                    $_REQUEST['by_streamlike_id'],
                    null,
                    null,
                    $cachetime,
                )
            ...
            $strSqlList = Pelican_Cache::fetch(
                'Service/Streamlike',
                array(
                    'list',
                    $streamlikeConfig['STREAMLIKE_COMPANY_ID'],
                    null,
                    null,
                    $_REQUEST['by_streamlike_keyword'],
                    $cachetime,
                )
            );
        }
    ...
    protected function getPhysicalList($dir)
    {
        /*
        ...
        while ($file = readdir($ressource)) {
            $pathinfo = pathinfo($file);
            if ($this->aAllowedExtensions[$_REQUEST['type']][strtolower(
                    $pathinfo['extension']
                )] || $_REQUEST['type'] == 'all'
            ) {
                $strSqlList[] = array(
    ...
    protected function getSearchSqlBase()
    {
        $oConnection = Pelican_Db::getInstance();
        $strSqlList = 'SELECT
                     m.'.Pelican::$config['FW_MEDIA_FIELD_ID'].' as "id",
                     SUBSTRING_INDEX( '.Pelican::$config['FW_MEDIA_FIELD_PATH'].", '/', -1) AS name,
                     SUBSTRING_INDEX( ".Pelican::$config['FW_MEDIA_FIELD_PATH'].", '/', -1) AS title,
                     ".Pelican::$config['FW_MEDIA_FIELD_PATH'].' as "path" ';
        ...
        if (Pelican::$config['FW_MEDIA_FIELD_CREATION_DATE']) {
            $strSqlList .= ', '.$oConnection->dateSqlToString(
                    Pelican::$config['FW_MEDIA_FIELD_CREATION_DATE']
                ).' as "'.Pelican::$config['FW_MEDIA_FIELD_CREATION_DATE'].'"';
    ...
    protected function filterSearchSessionParams()
    {
        ...
        if (isset($_GET['dimension'])) {
            // filtre sur les dimensions de l'image
            if ($_GET['dimension'] || $_GET['action'] == 'search') {
                $_SESSION[APP]['media_dimension'][$_REQUEST['zone']]['dimension'] = $_GET['dimension'];
            } else {
                unset($_SESSION[APP]['media_dimension'][$_REQUEST['zone']]);
            }
        }
    }

    protected function addImageFilter(&$indexBind, &$aBind)
    {
        $oConnection = Pelican_Db::getInstance();
        $strSqlList = '';
        // filtre ratio
        if ($_SESSION[APP]['media_ratio'][$_REQUEST['zone']]['ratio']) {
            $format = Pelican_Cache::fetch(
                'Media/MediaFormat',
                array(
                    'MEDIA_FORMAT_ID' => $_SESSION[APP]['media_ratio'][$_REQUEST['zone']]['ratio'],
                )
            );
            $searchRatio = $format['MEDIA_FORMAT_RATIO'];
            $searchRatioWithoutComma = (float) str_replace(',', '.', $searchRatio);
            $marge = 5;

            $margeFinal = self::calculMarge($searchRatioWithoutComma, $marge);
            $bind = $oConnection->setBindValue($aBind, $indexBind++, $margeFinal[0]);
            $bind2 = $oConnection->setBindValue($aBind, $indexBind++, $margeFinal[1]);
            $strSqlList .= 'and round(MEDIA_WIDTH/MEDIA_HEIGHT, 5) between '.$bind.' and '.$bind2;
        }
        // filtre par dimension
        if (!empty($_SESSION[APP]['media_dimension'][$_REQUEST['zone']]['dimension'])) {
            $format = Pelican_Cache::fetch(
                'Media/MediaFormat',
                array(
                    'MEDIA_FORMAT_ID' => $_SESSION[APP]['media_dimension'][$_REQUEST['zone']]['dimension'],
                )
            );

            if (!empty($format['MEDIA_FORMAT_WIDTH'])) {
                if ($format['MEDIA_FORMAT_WIDTH'] != 0) {
                    $strSqlList .= ' and MEDIA_WIDTH >= '.$format['MEDIA_FORMAT_WIDTH'];
                }
                if ($format['MEDIA_FORMAT_HEIGHT'] != 0) {
                    $strSqlList .= ' and MEDIA_HEIGHT >= '.$format['MEDIA_FORMAT_HEIGHT'];
                }
            }
        }
    ...
    protected function _makeList($dir)
    {
        ...
        $rows = 3;
        if ($_GET['filter_COUNT_ITEMS']) {
            $rows = intval($_GET['filter_COUNT_ITEMS']) / $cols;
        }
        $this->root = $_GET['root'];
         ...
         */
        if ($_REQUEST['type'] == 'streamlike') {
            $strSqlList = $this->getStreamlikeList();
        } else {
            if (Pelican::$config['FW_MEDIA_TABLE_NAME']) {
                $strSqlList = $this->getSearchSqlBase();
                $this->filterSearchSessionParams();
                $aBind = array();
                $indexBind = 1;
                if ($_SESSION[APP]['media_search'][$_REQUEST['zone']]) {

                    // resultat recherche par ratio
                    $strSqlList .= ', round('.Pelican::$config['FW_MEDIA_FIELD_WIDTH'].'/'.Pelican::$config['FW_MEDIA_FIELD_HEIGHT'].', 5) as ratio ';
                    $strSqlList .= ' from
                                                    '.Pelican::$config['FW_MEDIA_FOLDER_TABLE_NAME'].' c
                                                    INNER JOIN '.Pelican::$config['FW_MEDIA_TABLE_NAME'].' m ON m.MEDIA_DIRECTORY_ID=c.MEDIA_DIRECTORY_ID
                                                    LEFT JOIN #pref#_media_alt_translation t ON m.MEDIA_ID = t.MEDIA_ID
                                                    WHERE 1=1 ';
                    ...
                    if ($_SESSION[APP]['media_search'][$_REQUEST['zone']]['path']) {
                        $bind = $oConnection->setBindValue(
                            $aBind,
                            $indexBind++,
                            Pelican_Text::unhtmlentities($_SESSION[APP]['media_search'][$_REQUEST['zone']]['path']),
                            'string'
                        );
                        $strSqlList .= ' AND '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_PATH'].' like '.$oConnection->getConcatClause(
                                array(
                                    $bind,
                                    "'%'",
                                )
                    ...
                    if ($_SESSION[APP]['media_search'][$_REQUEST['zone']]['recherche']) {
                        $strSqlList .= ' AND (';
                        $searchFile = Pelican_Media::getSearchFile(
                            $_SESSION[APP]['media_search'][$_REQUEST['zone']]['recherche']
                        );
                        $strSqlList .= $oConnection->getSearchClauseLike(
                            't.TITLE',
                            $searchFile,
                            0,
                            ':'.$indexBind++,
                        ...
                        if (is_numeric($searchFile)) {
                            ...
                            $strSqlList .= 'm.'.Pelican::$config['FW_MEDIA_FIELD_ID'].'='.$bind;
                        }
                        if (!is_numeric($searchFile)) {
                            $strSqlList .= ' OR ';
                            $bind = $oConnection->setBindValue($aBind, $indexBind++, $searchFile, 'string');
                            $strSqlList .= Pelican::$config['FW_MEDIA_FIELD_PATH'].' like '.$oConnection->getConcatClause(
                                    array(
                                        "'%'",
                                        $bind,
                                    )
                    ...
                    if ($_REQUEST['type'] == 'image') {
                        $strSqlList .= $this->addImageFilter($indexBind, $aBind);
                    }

                    $strSqlList .= ' GROUP BY m.MEDIA_ID ';
                    $strSqlList .= ' ORDER BY ';
                    $strSqlList .= ' LIST_ORDER DESC, "id" desc';
                } else {
                    $bind = $oConnection->setBindValue($aBind, $indexBind++, $dir, 'string');
                    $strSqlList .= ' FROM '.Pelican::$config['FW_MEDIA_TABLE_NAME'].' m WHERE '.Pelican::$config['FW_MEDIA_FIELD_FOLDER_ID'].'='.$bind;
                    /*
                ...
                $strSqlList = $this->getPhysicalList($dir);
            }
        }
        switch ($_REQUEST['type']) {
            case 'youtube':
            ...
            case 'image': {
                ...
                if ($table->aTableValues && $table->aTableValues[0]) {
                    $preview .= $this->getFilterForMedia();
                    $preview .= $this->getJsForMediaFilter();
                    ...
                    foreach ($table->aTableValues as $image) {
                        ...
                        $img = $this->_createThumbnail(
                            ...
                            ($_REQUEST['type'] != 'youtube' && $_REQUEST['type'] != 'streamlike')
                        );
                        if (!$image['title']) {
                            $info = pathinfo($image['path']);
                            $image['title'] = str_replace('.'.$info['extension'], '', $info['basename']);
                        }
                        $preview .= '<td class="thumbnail_td" onclick="'.JSTARGET.".previewMedia('".rawurlencode(
                                $image['path']
                            )."','".$image['id']."',this);\"><center>";
                        $preview .= '<div class="thumbnail_div">'.str_replace(
                                'vignette',
                                $image['title'],
                                str_replace(
                                    '<img ',
                                    '<img onmousedown="'.JSTARGET.'.initDrag('.$image['id'].",'file');\" ",
                                    $img
                                )
                            ).'</div>'.Pelican_Media::reduceText($image['title'], 18);
                        $preview .= '</center></td>';
                    }
                    $preview .= '</tr>';
                } else {
            ...
            default: {
                ...
                $table->addImage(
                    'extension',
                    Pelican::$config['LIB_PATH'].Pelican::$config['LIB_FILE'].'/images/',
                    'path',
                    '5',
                    'center',
                    'extension',
                ...
                $table->addColumn('name', 'name', '95', '', '', 'tblmediath');
                $table->addRowEvent(
                    'onclick',
                    JSTARGET.'.previewMedia',
                    array(
                        'path' => 'path',
                        'id' => 'id',
                        '' => 'this=this',
     ...
     * @return string
     */
    protected function getFilterForMedia()
    {
        return '<table class="filtre" summary="Filtres"><tbody><tr><td width="30%">
                <center> '.t(
            'NDP_COUNT_ITEM_PAGE'
        ).' <select class="filtre" name="filter_COUNT_ITEMS" id="filter_COUNT_ITEMS" style="width:30%" onchange="filter_change(this)">'.$this->getOptionsForSelectFilter(
            $this->getParam('filter_COUNT_ITEMS')
        ).'</select>
                </center></td></tr></tbody>
                </table>
                <form id="filter_form" name="filter_form" method="get" action="/_/Index/child">
                <input name="tid" id="tid" value="'.$this->getParam('tid').'" type="hidden">
                <input name="root" id="root" value="'.$this->root.'" type="hidden">
                <input name="type" id="type" value="'.$this->getParam('type').'" type="hidden">
                <input name="lib" id="lib" value="'.$this->getParam('lib').'" type="hidden">
                <input name="format" id="format" value="'.$this->getParam('format').'" type="hidden">
                <input name="zone" id="zone" value="'.$this->getParam('zone').'" type="hidden">
                <input name="timestamp" id="timestamp" value="'.$this->getParam('timestamp').'" type="hidden">
                <input name="media" id="media" value="'.$this->getParam('media').'" type="hidden">
                <input name="filter_COUNT_ITEMS" id="filter_COUNT_ITEMS2" value="'.$this->getParam(
            'filter_COUNT_ITEMS'
        ).'" type="hidden"></form>
                </form>';
    }
     ...
     * @return type
     */
    protected function getOptionsForSelectFilter($selected = '')
    {
        $options = '';
        foreach ($this->getItemCountFilter() as $option) {
            $isSelected = '';
            if ($option == $selected) {
                $isSelected = 'selected';
            }
     ...
     * @return string
     */
    protected function getJsForMediaFilter()
    {
        return "<script type='text/javascript'>
        function filter_change(obj, name) {
            if (!name) {
     ...
     * @return array
     */
    protected function getItemCountFilter()
    {
        return [9, 15, 30, 60, 90];
    }
    ...
    protected function _createThumbnail($file, $values = array(), $altText = '', $format = true)
    {
        ...
        if (!preg_match('/https:\/\//', $file) && !preg_match('/http:\/\//', $file) && !preg_match(
                '/ftp:\/\//',
                $file
            ) && !preg_match('/mms:\/\//', $file)
        ) {
    ...
    {
        $data = Ndp_Media::checkMediaUsageDetail($id);
        ...
        $table2->addColumn(t('URL'), 'page_clear_url', '5', 'left', '', 'tblheader');
    ...
    /**
     * @param $type
     * @param null $mediaSize
     * @param null $name
     *
     * @return mixed|mixed|null|string
     */
    public function checkMaxSize($type, $mediaSize = null, $name = null)
    {
        $message = null;

        if ($mediaSize > Pelican::$config['MAX_SIZE'][strtoupper($type)]['size']) {
            $message = t('MAX_SIZE_'.strtoupper($type));
            $message .= '\r\n';
            $message .= sprintf(t('MEDIA_FILE_CAUSES_UPLOAD_ERROR').': %s', $name);
        }
