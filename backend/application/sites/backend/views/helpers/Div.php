...
class Backoffice_Div_Helper
{
    ...
    public static function top($name = '')
    {
        ...
        $title [] = Pelican_Html::img(array(
            'id' => "header_logout",
            'src' => self::$skin."/images/logout.gif",
            'alt' => "D&eacute;connexion",
            'onclick' => "document.location.href=&#39;/_/Index/login&#39;"
        ));
        ...
        if (Pelican::$config ["ENVIRONNEMENT"]) {
            $title [] = Pelican_Html::u(array('id' => "env-label"),t(Pelican::$config ["ENVIRONNEMENT"]));
            $title [] = "";
        }
        ...
        $header_tag [] = Pelican_Html::div(array('id' => "header_left"),
            Pelican_Html::img(array('src' => self::$skin."/images/top_left.gif", 'alt' => "")));
        $header_tag [] = Pelican_Html::div(array('id' => "header_right"),
            Pelican_Html::img(array('src' => self::$skin."/images/top_right.gif", 'alt' => "")));
        $return = implode("\n", $header_tag);

        // Sélection du site courant
        if ($_SESSION [APP] ["htmlComboSite"]) {
            $return .= Pelican_Html::script(array('type' => "text/javascript"),
                "document.getElementById(\"SITE_ID\").value='".$_SESSION [APP] ["PROFILE_ID"]."_".$_SESSION [APP] ['SITE_ID']."'");
        }
    ...
    {
        $footer [] = Pelican_Html::div(array(id => "footer_left"),
            Pelican_Html::img(array(src => self::$skin."/images/bottom_left.gif", alt => "")));
        $footer [] = Pelican_Html::div(array(id => "footer_middle"), "&nbsp;");
        $footer [] = Pelican_Html::div(array(id => "footer_right"),
            Pelican_Html::img(array(src => self::$skin."/images/bottom_right.gif", alt => "")));
    ...
    {
        $previewAction = "clickButton('preview');";
        $return = Backoffice_Button_Helper::display("button_mutualisation", t('FORM_BUTTON_COPY'),
            self::$skin."/images/button_mutualisation.gif", "clickButton('mutualisation');");

        if (Psa_User_Backoffice::isProfileTranslator()) {
            $previewAction = "";
        }

        $return .= Backoffice_Button_Helper::display("button_preview", t('Preview'),
            self::$skin . "/images/button_preview.gif", $previewAction);

        $aStates = Pelican_Cache::fetch("Backend/State");
        if ($aStates) {
            foreach ($aStates as $state) {
                $action = "clickButton('state_".$state ["id"]."');";
                if (Psa_User_Backoffice::isProfileTranslator()) {
                    if (in_array($state ["id"], Pelican::$config['PROFILE']['TRANSLATOR_EXCLUDE_STATES'])) {
                        $action = "";
                    }
                }
                $return .= Backoffice_Button_Helper::display("button_state_".$state ["id"], $state ["lib2"],
                    self::$skin."/images/tree_workflow_detail.gif", $action);
            }
        }
        $return .= Backoffice_Button_Helper::display("button_add", t('POPUP_LABEL_ADD'),
            self::$skin."/images/button_add.gif", "clickButton('add');");
        $return .= Backoffice_Button_Helper::display("button_save", t('EDITOR_SAVE'),
            self::$skin."/images/button_save.gif", "clickButton('save');");
        $return .= Backoffice_Button_Helper::display("button_delete", t('POPUP_LABEL_DEL'),
            self::$skin."/images/button_del.gif", "clickButton('delete');");
        $return .= Backoffice_Button_Helper::display("button_back", t('POPUP_BUTTON_BACK'),
            self::$skin."/images/button_back.gif", ($popup ? "goBack();" : "clickButton('back');"));

        // Médiathèque
        $return .= Backoffice_Button_Helper::display("buttonAddFile", t('POPUP_LABEL_ADD'),
            self::$skin."/images/button_add.gif", "setAction('add','file');");
        $return .= Backoffice_Button_Helper::display("buttonDelFile", t('POPUP_LABEL_DEL'),
            self::$skin."/images/button_delfile.gif", "setAction('del','file');");
        $return .= Backoffice_Button_Helper::display("buttonBack", t('POPUP_BUTTON_BACK'),
            self::$skin."/images/button_back.gif", "top.goBack();");
    ...
    public static function leftBottom()
    {
        ...
        $return .= Backoffice_Button_Helper::display("button_up", "", self::$skin."/images/arrow/up.gif",
            (Psa_User_Backoffice::isProfileTranslator() ? "" : "orderFolderHmvc(-1);"));
        $return .= Backoffice_Button_Helper::display("button_down", "", self::$skin."/images/arrow/down.gif",
            (Psa_User_Backoffice::isProfileTranslator() ? "" : "orderFolderHmvc(1);"));
        $return .= "</div>";

        $return .= Backoffice_Button_Helper::display("buttonAddFolder", t('Aj.'),
            self::$skin."/images/button_addfolder.gif", "setAction('add','folder');");
        $return .= Backoffice_Button_Helper::display("buttonEditFolder", t('Ed.'),
            self::$skin."/images/button_editfolder.gif", "setAction('edit','folder');");
        $return .= Backoffice_Button_Helper::display("buttonDelFolder", t('Sup.'),
            self::$skin."/images/button_delfolder.gif", "setAction('del','folder');");
        ...
        $return .= Backoffice_Button_Helper::display("button_addpage", t('Aj.'),
            self::$skin."/images/button_addfolder.gif",
            (Psa_User_Backoffice::isProfileTranslator() ? "" : "menu(lastMenu['tid'], lastMenu['tc'], -2, lastMenu['id']);"));
        //}
        $return .= Backoffice_Button_Helper::display("button_deletepage", t('Sup.'),
            self::$skin."/images/button_delfolder.gif",
            (Psa_User_Backoffice::isProfileTranslator() ? "" : "menu(lastMenu['tid'], lastMenu['tc'], lastMenu['id'], lastMenu['id'], 'true');"));
    ...
    {
        $return = Backoffice_Button_Helper::display("button_mutualisation", t('EDITOR_COPY'),
            self::$skin."/images/button_mutualisation.gif", "clickButton('mutualisation');");
        $return .= Backoffice_Button_Helper::display("button_preview", t('Preview'),
            self::$skin."/images/button_preview.gif", "clickButton('preview');");
        $aStates = Pelican_Cache::fetch("Backend/State");
        if ($aStates) {
            foreach ($aStates as $state) {
                $return .= Backoffice_Button_Helper::display("button_state_".$state ["id"], $state ["lib"],
                    self::$skin."/images/tree_workflow_detail.gif", "clickButton('state_".$state ["id"]."');");
            }
        }
        $return .= Backoffice_Button_Helper::display("button_addpage", t('Aj.'), "", "");
        $return .= Backoffice_Button_Helper::display("button_deletepage", t('Sup.'), "", "");
    ...
    public static function getTab($aValues)
    {
        ...
        foreach ($aValues as $onglet) {
            if ($onglet ["id"]) {
                ...
                $oTab->addTab(t($labelTrad), "onglet_top_".$onglet ["id"], ($_GET ["view"] == "O_".$onglet ["id"]),
                    Pelican::$config ["PAGE_INDEX_PATH"]."?view=O_".$onglet ["id"], "", $onglet ["volet_gauche"]);
            }
        }
        $return = $oTab->getTabs();
