...
$user = new Psa_Dsin_User($login,$password);
if ($user->login())
{
	//connexion etablie
	....
...
Les droits sont : "ADMINISTRATEUR", "CONTRIBUTEUR", "WEBMASTER", "IMPORTATEUR" (définit dans des constantes ROLE_xxx de la classe Psa_Dsin_User)
Exemple :
...
Les différent droits sont donnés dans les const de Psa_Dsin_User.
...
Les différent métiers sont donnés dans les const de Psa_Dsin_User.
