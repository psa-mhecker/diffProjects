...
class LightOpenId
{
    ...
    public function set_proxy($proxy)
    {
        if (!empty($proxy)) {
            ...
            if ($proxy && !empty($proxy['proxy_host'])) {
                // Make sure that a valid port number is specified.
                if (array_key_exists('proxy_port', $proxy)) {
                    if (!is_int($proxy['proxy_port'])) {
                        $proxy['proxy_port'] = is_numeric($proxy['proxy_port']) ? intval($proxy['proxy_port']) : 0;
                    }
    ...
    protected function request_curl($url, $method = 'GET', $params = array(), $update_claimed_id)
    {
        ...
        if (!empty($this->proxy)) {
            curl_setopt($curl, CURLOPT_PROXY, $this->proxy['proxy_host']);

            if (!empty($this->proxy['proxy_port'])) {
                curl_setopt($curl, CURLOPT_PROXYPORT, $this->proxy['proxy_port']);
            }

            if (!empty($this->proxy['proxy_login'])) {
                curl_setopt($curl, CURLOPT_PROXYUSERPWD, $this->proxy['proxy_login'].':'.$this->proxy['proxy_password']);
            }
        }
    ...
    protected function proxy_url()
    {
        ...
        if (!empty($this->proxy)) {
            $result = $this->proxy['proxy_host'];

            if (!empty($this->proxy['proxy_port'])) {
                $result = $result.':'.$this->proxy['proxy_port'];
            }

            if (!empty($this->proxy['proxy_login'])) {
                $result = $this->proxy['proxy_login'].':'.$this->proxy['proxy_password'].'@'.$result;
            }
            ...
            return empty($result) ? false : $result[0];
                ...
                $ns = preg_quote('http://specs.openid.net/auth/2.0/', '#');
                            ...
                            || strpos($content, '<Type>http://openid.net/extensions/sreg/1.1</Type>');
                                    ...
                                    $this->server = html_entity_decode($server);
