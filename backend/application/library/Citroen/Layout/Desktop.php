                        ...
                        $data = array();
                            ...
                            if (is_array($profileUser) && count($profileUser)>0) {
                                ...
                                foreach ($listData as $key => $oneData) {
                                    ...
                                    if ((in_array($key, $profileUser, true) && Citroen_Cache::isPubliePerso($oneData)) ||
                                        (
                                            !empty($explodeKey)
  ...
  default: {
    ...
    public static function getModeDeploye($data)
    {
        ...
        if (!empty(Pelican::$config['DEPLOYABLE_BLOC']) && !$bMobile) {
            ...
            foreach (Pelican::$config['DEPLOYABLE_BLOC'] as $kDeployable => $deployable) {
                $aReponse .= '
                <div class="secret" id="deployable_'.$kDeployable.'">
                    <div class="closer"></div>';
                ...
                if ($deployable == 'PDV') {
                    ...
                    $aReponse .= Pelican_Request::cachedCall(trim($dataDeploy["ZONE_FO_PATH"]), $dataDeploy, $cache, $data["ZONE_CACHE_TIME"]);
                } else {
                    $aZoneDeploy = Pelican_Cache::fetch("Zone", array(Pelican::$config['ZONE']['FORMULAIRE']));
