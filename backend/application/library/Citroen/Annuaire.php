...
class Annuaire
{
     ...
     */
    public static function getDealerList($iLat, $iLon, $sPays, $sLocale, $iRmax, $aRequest, $nbmax, $minpdv, $mindvn)
    {
        if ($minpdv != '' && $mindvn != '') {
            $searchtype = 'spiral';
        } else {
    ...
    public static function getDealer($id, $sPays, $sLocale)
    {
        ...
        if (is_array($aResponse) && count($aResponse) > 0) {
            ...
            if ($aResponse['SiteGeo']) {
                $aDealer["id"] = $aResponse['SiteGeo'];
            }
            if ($aResponse['Image']) {
                $aDealer["media"] = $aResponse['Image'];
            }
            if ($aResponse['Name']) {
                $aDealer["name"] = utf8_encode_without_cyrilique($aResponse['Name'], $sLocale);
            }
            if ($sAddress) {
                $aDealer["address"] = utf8_encode_without_cyrilique($sAddress, $sLocale);
            }
            if ($aResponse['Phones']['PhoneNumber']) {
                $aDealer["phone"] = $aResponse['Phones']['PhoneNumber'];
            }
            if ($aResponse['FaxNumber']) {
                $aDealer["fax"] = $aResponse['FaxNumber'];
            }
            if ($aResponse['Emails']['Email']) {
                $aDealer["email"] = $aResponse['Emails']['Email'];
            }
            $aDealer["web"] = "";
            ...
            if ($sHoraires) {
                $aDealer["timetable"] = utf8_encode_without_cyrilique($sHoraires, $sLocale);
            }
            if ($aResponse['DistanceFromPoint']) {
                $aDealer["kilometrage"] = $aResponse['DistanceFromPoint'];
            }
            if ($aServiceDealer) {
                $aDealer["services"] = $aServiceDealer;
            }
            if ($aServiceMobile) {
                $aDealer["servicesMob"] = $aServiceMobile;
            }
            if ($serviceList) {
                $aDealer["serviceList"] = $serviceList;
            }
            if ($aBenefits) {
                $aDealer["benefits"] = $aBenefits;
            }
            if ($aResponse['WelcomeMessage']) {
                $aDealer["welcome"] = utf8_encode_without_cyrilique($aResponse['WelcomeMessage'], $sLocale);
            }
            if ($aContacts) {
                $aDealer["contacts"] = $aContacts;
            }
            if ($aResponse['Address']) {
                $aDealer["addressDetail"] = $aResponse['Address'];
            }
