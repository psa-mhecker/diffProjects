...
'use strict';
  ...
  if( $('html').hasClass('touch') && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
    ...
    $el.find('.form-control').on('click', function(){
      ...
      if ($('.popinopen').length) {
        /*$('html, body').animate({
          scrollTop: parseFloat($(this).offset().top)
        }, 1500);*/
      }
      else{
        $('html, body').animate({ scrollTop: $currentLabel.offset().top  }, 0);
      }
    });// + parseFloat($('.popin--wrapper').scrollTop()) - 28*/
...
*/
  ...
  function validation($that, inputValue){
    if( $that.attr('type') === 'email') {
      ...
      if( !$that.attr('checked')) {
        errors++;
        $that.parent('.input-checkbox').addClass('erreurbox');
        if( $that.attr('data-msg-required') !== ''){
          $('.confirm-mentions').append('<span class="error-msg">'+ $that.attr('data-msg-required') +'</span>');
        }
      }
      else {
        $that.parent('.input-checkbox').removeClass('erreurbox');
  ...
  function formvalide(){
      ...
      $('input[data-required="true"]').each(function(){
        ...
        if( i === inputLength ) { /* CallBack */
          if( $('.error-msg').length === 0 ) {
            // TODO : appel AJAX pour enregistrer les donn̩es du formulaire au back
    ...
    $('.confirm-mentions label').on('click', function(e){
      e.preventDefault();
      var $par = $(this).parent('.input-checkbox:eq(0)');
