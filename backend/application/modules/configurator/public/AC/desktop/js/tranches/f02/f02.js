...
viewManager.prototype = {
	init:function($root){
		...
		oThis.$canvasModel = $('<canvas id="canvas" width="100%" height="315px"></canvas>');
...
oThis.debug('Slick initialized ');
		...
		var $slickDotsContainer = $viewSlick.find('.slick-dots'), 
			$slickDots = $slickDotsContainer.find('li'),
			$slickArrows = $viewSlick.find('.slick-arrow');

		if($slickArrows.length>0 && $slickDotsContainer.length>0){
			$slickArrows.each(function(){
				var $arrow = $(this);
				if($arrow.hasClass('slick-prev')){
					$arrow.prependTo($slickDotsContainer);
				}
				if($arrow.hasClass('slick-next')){
					$arrow.appendTo($slickDotsContainer);
				}
			});
		}
	}, 
