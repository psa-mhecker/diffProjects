...
class Spreadsheet_Excel_Reader {
	...
	function colwidth($col, $sheet = 0) {
		// Col width is actually the width of the number 0. So we have to
		// estimate and come close
	...
	function _format_value($format, $num, $f) {
		...
		// In Excel formats, "_" is used to add spacing, which we can't do in
		// HTML
			...
			switch ($code) {
				case SPREADSHEET_EXCEL_READER_TYPE_SST :
					...
					for($i = 0; $i < $uniqueStrings; $i ++) {
						...
						// $retstr = ($asciiEncoding) ? $retstr :
						// $this->_encodeUTF16($retstr);
						// $retstr =
						// $this->seems_utf8($retstr)?$retstr:utf8_encode($retstr);
					...
					if (array_key_exists ( $indexCode, $this->dateFormats )) {
						...
						if ($indexCode > 0) {
							...
							if ($formatstr != "") {
								...
								if (preg_match ( "/[^hmsday\/\-:\s\\\,AMP]/i", $tmp ) == 0) { // found
								                                                           // day and
								                                                           // time
								                                                           // format
									$isdate = TRUE;
									...
									// m/mm are used for both minutes and months
									// - oh SNAP!
									// This mess tries to fix for that.
									// 'm' == minutes only if following h/hh or
									// preceding s/ss
						...
						if ($isdate) {
							...
							// If the format string has a 0 or # in it, we'll
		...
		while ( $cont ) {
			...
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_NUMBER :
					$row = ord ( $data [$spos] ) | ord ( $data [$spos + 1] ) << 8;
					$column = ord ( $data [$spos + 2] ) | ord ( $data [$spos + 3] ) << 8;
					$tmp = unpack ( "ddouble", substr ( $data, $spos + 6, 8 ) ); // It
					                                                       // machine machine
					                                                       // dependent
					if ($this->isDate ( $spos )) {
						$numValue = $tmp ['double'];
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FORMULA2 :
					...
					if ((ord ( $data [$spos + 6] ) == 0) && (ord ( $data [$spos + 12] ) == 255) && (ord ( $data [$spos + 13] ) == 255)) {
						...
						// This row/col are stored to be referenced in that
						// record
	...
	function _getCellDetails($spos, $numValue, $column) {
		...
		if ($type == 'date') {
			// See
			// http://groups.google.com/group/php-excel-reader-discuss/browse_frm/thread/9c3f9790d12d8e10/f2045c2369ac79de
			$rectype = 'date';
			...
			$raw = $numValue;
			$fractionalDay = $numValue - floor ( $numValue ) + .0000001; // The
			                                                          // .0000001 is to
			                                                          // fix for
			                                                          // php/excel
			                                                          // fractional diffs
