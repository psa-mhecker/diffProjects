...
class Google_CurlIO extends Google_IO {
  ...
  public function makeRequest(Google_HttpRequest $request) {
    ...
    curl_close($ch);
    $useProxy = (@$this->curlParams[CURLOPT_PROXYTYPE] === CURLPROXY_HTTP);
    ...
    list($responseHeaders, $responseBody) =
          self::parseHttpResponse($respData, $respHeaderSize,$useProxy);
   ...
   * @return array
   */
  private static function parseHttpResponse($respData, $headerSize,$useProxy =false) {
    if ($useProxy)	{
        $ver = curl_version();
        $versionNum = $ver['version_number'];
        // hex for version 7.31.0
        if ($versionNum < 0x071F00)	{			
         $p = strpos($respData, "\r\n\r\n");
         $respData = substr($respData, $p+4);			
        }		
    }
