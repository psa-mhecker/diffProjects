...
class Minify_HTML
{
    ...
    public static function minify($html, $options = array())
    {
        ...
        $html = preg_replace_callback(
            '/(\\s*)(<script\\b[^>]*?>)([\\s\\S]*?)<\\/script>(\\s*)/i', array(self::$className, '_removeScriptCB'), $html);

        // replace STYLEs (and minify) with placeholders
        $html = preg_replace_callback(
            '/\\s*(<style\\b[^>]*?>)([\\s\\S]*?)<\\/style>\\s*/i', array(self::$className, '_removeStyleCB'), $html);

        // remove Pelican_Html comments (not containing IE conditional comments).
        $html = preg_replace_callback(
            '/<!--([\\s\\S]*?)-->/', array(self::$className, '_commentCB'), $html);
        ...
        $html = preg_replace_callback(
            '/\\s*(<textarea\\b[^>]*?>[\\s\\S]*?<\\/textarea>)\\s*/i', array(self::$className, '_removeTaCB'), $html);
        ...
        // fill placeholders
        $html = str_replace(
            array_keys(self::$_placeholders), array_values(self::$_placeholders), $html
        );
        self::$_placeholders = array();
    ...
    protected static function _removeScriptCB($m)
    {
        ...
        // remove Pelican_Html comments (and ending "//" if present)
