...
class Minify_ImportProcessor
{
    ...
    private function _getContent($file)
    {
        ...
        if (self::$_isCss) {
            ...
            $content = preg_replace_callback(
                '/url\\(\\s*([^\\)\\s]+)\\s*\\)/', array($this, '_urlCB'), $content
            );
        }
    ...
    private function _importCB($m)
    {
        ...
        if (strpos($url, '://') > 0) {
            // protocol, leave in place for CSS, Pelican_Index_Comment for JS
            return self::$_isCss
                ? $m[0]
        ...
        if ('' === $content) {
            // failed. leave in place for CSS, Pelican_Index_Comment for JS
            return self::$_isCss
                ? $m[0]
