...
/**
 * Minify - Combines, minifies, and application/caches JavaScript and CSS files on demand.
 *
...
class Minify
{
    ...
    /**
     * Specify a Pelican_Cache object (with identical interface as Minify_Cache_File) or
     * a path to use with Minify_Cache_File.
     *
     * If not called, Minify will not use a Pelican_Cache and, for each 200 response, will
     ...
     * headers, allowing shared application/caches to Pelican_Cache the output. (default true)
     *
    ...
    public static function serve($controller, $options = array())
    {
        ...
        if ($cg->cacheIsValid) {
            // client's Pelican_Cache is valid
            if (! self::$_options['quiet']) {
                $cg->sendHeaders();
        ...
        if (null !== self::$_cache) {
            // using cache
            // the goal is to use only the Pelican_Cache methods to sniff the length and
            ...
            // check Pelican_Cache for valid entry
            $cacheIsReady = self::$_cache->isValid($fullCacheId, self::$_options['lastModifiedTime']);
            if ($cacheIsReady) {
                $cacheContentLength = self::$_cache->getSize($fullCacheId);
            } else {
                // generate & Pelican_Cache content
    ...
    public static function combine($sources, $options = array())
    {
        ...
        $options = array_merge(array(
            'files' => (array) $sources, 'quiet' => true, 'encodeMethod' => '', 'lastModifiedTime' => 0,
        ), $options);
    ...
    public static function setDocRoot($unsetPathInfo = false)
    {
        if (isset($_SERVER['SERVER_SOFTWARE'])
            ...
            $_SERVER['DOCUMENT_ROOT'] = substr(
                $_SERVER['PATH_TRANSLATED'], 0, strlen($_SERVER['PATH_TRANSLATED']) - strlen($_SERVER['SCRIPT_NAME'])
            );
            if ($unsetPathInfo) {
                unset($_SERVER['PATH_INFO']);
            }
            require_once 'Minify/Logger.php';
            Minify_Pelican_Log::log("setDocRoot() set DOCUMENT_ROOT to \"{$_SERVER['DOCUMENT_ROOT']}\"");
        }
    }

    /**
     * @var mixed Minify_Cache_* object or null (i.e. no server Pelican_Cache is used)
     */
    ...
    protected static function _combineMinify()
    {
        ...
        // trailing single line Pelican_Index_Comment is terminated
    ...
    /**
     * Make a unique Pelican_Cache id for for this request.
     *
    ...
    protected static function _getCacheId()
    {
        return md5(serialize(array(
            Minify_Source::getDigest(self::$_controller->sources), self::$_options['minifiers'], self::$_options['minifierOptions'], self::$_options['postprocessor'], self::$_options['bubbleCssImports'],
        )));
    }
    ...
    protected static function _handleCssImports($css)
    {
        if (self::$_options['bubbleCssImports']) {
            ...
            // remove comments so we don't mistake { in a Pelican_Index_Comment as a block
