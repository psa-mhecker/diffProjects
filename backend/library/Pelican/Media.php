        ...
        if (Pelican::$config["FW_MEDIA_TABLE_NAME"] && Pelican::$config["FW_MEDIA_USAGE"] && !empty($id)) {
            ...
            foreach (Pelican::$config["FW_MEDIA_USAGE"] as $table) {
                if ($table == "view_content_version") {
                    $count = $oConnection->queryItem(
                        "SELECT count(1) FROM #pref#_content_version WHERE MEDIA_ID=:ID",
                        array(":ID" => $id)
                    );
                } else {
                    $count = $oConnection->queryItem(
                        "SELECT count(1) FROM ".$table." WHERE ".Pelican::$config["FW_MEDIA_FIELD_ID"]."=:ID",
                        array(":ID" => $id)
                    );
                }
                if ($count) {
        ...
        if ($swf) {
            ...
            switch ($info["extension"]) {
                case "zip": {
                    ...
                    $params["file"] = str_replace(
                        ".".$info["extension"],
                        "",
                        str_replace("/videos", "", $swf["MEDIA_PATH"])
                    );
                    $params["debit"] = 1;
                    $params["vidPlay"] = 'true';
                    break;
                ...
                case "wmv": {
                    ...
                    if (preg_match('/^http:\/\//', $file) || preg_match('/^ftp:\/\//', $file) || preg_match(
                            '/^mms:\/\//',
                            $file
                        )
                    ) {
            ...
            switch ($info["extension"]) {
                case "mov":
                    ...
                    $param .= empty($autoplay) ? Pelican_Html::param(
                        array('name' => 'autoplay', 'value' => 'false')
                    ) : Pelican_Html::param(array('name' => 'autoplay', 'value' => 'true'));
                    $auto = empty($autoplay) ? array('autoplay', 'false') : array('autoplay', 'true');
                    $param .= $subtitle ? Pelican_Html::param(
                        array('name' => 'captions', 'value' => '')
                    ) : Pelican_Html::param(array('name' => 'captions', 'value' => $subtitle));
                    ...
                    $param2 .= empty($autoplay) ? Pelican_Html::param(
                        array('name' => 'autoplay', 'value' => 'false')
                    ) : Pelican_Html::param(array('name' => 'autoplay', 'value' => 'true'));
                    $param2 .= $subtitle ? Pelican_Html::param(
                        array('name' => 'captions', 'value' => '')
                    ) : Pelican_Html::param(array('name' => 'captions', 'value' => $subtitle));
                    $param2 = str_replace('></param>', '/>', $param2);
                    $param .= Pelican_Html::object(
                        array('type' => 'video/quicktime', 'width' => $width, 'height' => $height, 'data' => $file),
                        $param2
                    );
                    $param = str_replace('></param>', '/>', $param);
                    switch ($front) {
                        case false:
                            $return = Pelican_Html::object(
                                array(
                ...
                case "rm":
                    ...
                    $param .= empty($autoplay) ? Pelican_Html::param(
                        array('name' => 'autoStart', 'value' => '0')
                    ) : Pelican_Html::param(array('name' => 'autoStart', 'value' => '1'));
                    $param .= $subtitle ? Pelican_Html::param(
                        array('name' => 'captions', 'value' => '')
                    ) : Pelican_Html::param(array('name' => 'captions', 'value' => $subtitle));
                    $param .= Pelican_Html::param(array('name' => 'src', 'value' => $file));
                    ...
                    $param .= Pelican_Html::embed(
                        array(
                            ...
                            'console' => 'video',
                            $auto[0] => $auto[1],
                            $subtitle_param[0] => $subtitle_param[1]
                        )
                    );
                    switch ($front) {
                        case false:
                ...
                case "flv":
                    ...
                    switch ($front) {
                        case false:
                            ...
                            $return = Pelican_Html::div(
                                array(
                                    id => "player".$swf["MEDIA_ID"]."_".$size,
                                    style => "height:".$height."px;width:".$width."px;text-align:center;"
                                )
                            );
                            break;
                        case true:
                ...
                case "webm":
                    ...
                    $return = Pelican_Html::_tag1(
                        'video',
                        array(array(controls => 'controls', width => $width, height => $height), $return)
                    );
