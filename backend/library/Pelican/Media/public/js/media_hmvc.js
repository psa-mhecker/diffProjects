...
if (windowArguments) {
	if (windowArguments["mediaType"]) {
		if (windowArguments["mediaType"] != 'image' && windowArguments["mediaType"] != 'file' && windowArguments["mediaType"] != 'flash' && windowArguments["mediaType"] != 'video' && windowArguments["mediaType"] != 'streamlike' && windowArguments["mediaType"] != 'video' && windowArguments["mediaType"] != 'all') {
			windowArguments["mediaType"] = 'image';
		}
	} else {
		windowArguments["mediaType"] = 'image';
...
function select() {
	...
	if (windowArguments) {
		if (windowArguments["mediaZone"] == "editor") {
			...
			buildTag();
			window.returnValue = decodeURIComponent(current.mediaTag);
		} else {
            mediaPath = decodeURIComponent(current.mediaPath);
			if (current.mediaId) {
                preview = "<a href='" + httpMediaDir + mediaPath + "' target='_blank' >" + decodeURIComponent(current.previewTag) + "</a>";
                if(current.mediaType == "streamlike") {
                    preview = "<img src='" + mediaPath + "'/>";
                }
			}
            if(windowArguments["crop"]) {
                opener.returnPopupMediaCroped( current.mediaId, httpMediaDir+mediaPath, windowArguments["Field"], windowArguments["Div"]);
            } else {
                // should work for firefox only and new IE is compatible
                opener.returnPopupMedia( current.mediaId, preview,mediaPath,  windowArguments["Field"], windowArguments["Div"]);
            }
		}
	}
	top.close();
