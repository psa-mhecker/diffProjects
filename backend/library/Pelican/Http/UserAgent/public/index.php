...
if ($_GET['updateDatabase']) {
    ...
    if ($_GET['brand']) {
        $brandName = filterClean($_GET['brand']);
        $_SESSION['HTTP_USER_AGENT'] = null;
    }
    if ($_GET['model']) {
        $modelName = filterClean($_GET['model']);
        $_SESSION['HTTP_USER_AGENT'] = null;
    }
    if ($_GET['markup']) {
        $markup = filterClean($_GET['markup']);
    }
...
function filterClean($input = NULL){

    $output = addslashes(htmlspecialchars(rawurldecode($input)));
...
class Device_Explorer
{
    ...
    {
        $key = key($data);
        $this->filter = $key."='".filterClean($data[$key])."'";
    }
