...
class Pelican_Cache_Media extends Pelican_Cache
{
     ...
     * @var string
     */
    protected $colorComplete;
    /**
     * @var bool
     */
    protected $_binaryCache;
     ...
     * @access public
     *
     * @param string $path  Chemin absolu de l'image originale
     * @param int|string $format  (option) Id de format d'image
     * @param string $crop (option) Coordonn�es de recadrage
     * @param bool $bypass (option) Pour éviter l'utilisation des formats "forcés"  (lors de tests découpages)
     * @param int|string $lifeTime  (option) Constantes de dur�e de vie du fichier de  Pelican_Cache
     * @param string $colorComplete  (option)
     * @param string $width  (option)
     * @param string $height (option)
     * @param string $extension (option)
     *
     */
    public function __construct($path, $format = "", $crop = "", $bypass = false, $lifeTime = "", $colorComplete = "", $width = "", $height = "", $extension = "")
    {
    ...
    public function getValue()
    {
        ...
        if ($this->_format || $this->_format === "0" || ($this->params[2] && $this->params[3])) {
            ...
            $im->setOpt('antialias', ' ');
            if (!empty(Pelican::$config['IMAGE_MAGICK']['OPT']['QUALITY'])) {
                if (Pelican::$config['IMAGE_MAGICK']['OPT']['QUALITY'] > 0) {
            $im->setOpt('quality', escapeshellcmd(Pelican::$config['IMAGE_MAGICK']['OPT']['QUALITY']));
                }
            }
            if (!empty(Pelican::$config['IMAGE_MAGICK']['OPT']['TYPE'])) {
                if (Pelican::$config['IMAGE_MAGICK']['OPT']['TYPE'] > 0) {
            $im->setOpt('type', Pelican::$config['IMAGE_MAGICK']['OPT']['TYPE']);
                }
            }
            if (!empty(Pelican::$config['IMAGE_MAGICK']['OPT']['UNSHARP'])) {
                if (Pelican::$config['IMAGE_MAGICK']['OPT']['UNSHARP'] > 0) {
