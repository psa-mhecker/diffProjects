...
class Pelican_Request extends Zend_Controller_Request_Http
{
    ...
    public function getContainer()
    {
        if(!isset($this->container)) {
            $this->container = Pelican_Application::getContainer();
        }
    ...
    {
        if(!isset($this->pimple)) {
            $this->pimple = Pelican_Application::getPimple();
        }
        ...
        try {
            if (! class_exists($this->controllerName)) {
                if (file_exists($this->controllerPath)) {
                    ...
                    $this->setHeaders($_SERVER["SERVER_PROTOCOL"]." 404 Not Found");
...
border-color:  #297CB4 #083F6F;background-color:purple;border-bottom : 1px solid purple;font-weight:bold;color:white;",
    ...
    public function cachedCall($uri, $localParams = array(), $activeCache = false, $lifetime = 30)
    {
        ...
        if ($activeCache && $lifetime > 0) {
            self::$cacheUsed = 1;
            $timestamp = Pelican_Cache::getSecondTimeStep($lifetime);
            $idcache = Pelican_View::getCacheId($idcache, array(
                $_SESSION[APP]['LANGUE_ID'],
                Pelican_Controller::isMobile(),
                Pelican::$config["SERVER_PROTOCOL"].Pelican::$config["HTTP_HOST"]
            ));
            $responseArray = Pelican_Cache::fetch('Request', array(
                $uri,
                serialize($localParams),
                $idcache,
                $timestamp,
            ));
            $response = $responseArray['content'];
            if (!empty($responseArray['head']) && empty(Pelican::$config['CATCH_EXTERNAL_HEAD'])) {
                Pelican_Factory::getInstance('View')->getHead()
                    ->setAddon("\n<!--BEGIN WEBS-->\n" . $responseArray['head'] . "\n<!--END WEBS-->\n");
            }
            if (!empty(Pelican::$config['CATCH_EXTERNAL_HEAD'])) {
                Pelican::$config['CATCH_EXTERNAL_HEAD'] = '';
            }
        } else {
     ...
     * @return string
     */
    public static function getCacheId($idcache = "", $addon = array())
    {
        $return = $idcache;
        if (is_array($return)) {
            ksort($return);
