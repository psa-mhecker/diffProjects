...
define('gamme_csv_memory_limit','2500M');
...
class Gamme  
{
    ...
    private function setGammeSites()
    {
        ...
        $this->aSites = null;
         ...
         $aSites = \Pelican_Cache::fetch('Citroen/SiteBySiteCode', array(null, \Pelican_Cache::getTimeStep(DAY)));
         if (is_array($aSites)){
            $this->aSites = $aSites;
        }
    ...
    public function importAllCSVData(){
                ...
                if (is_array($aLangues) && !empty($aLangues)) {
                    foreach($aLangues as $langue) {
                        ...
                        foreach($TypeVehicules as $type) {                            
                            ...
                            $this->importCSVDataToObject($sites['SITE_ID'], $langue['LANGUE_ID'], $type);
        ...
        \Pelican_Cache::clean("Citroen/Frontend/Citroen/VehiculeShowroomById");
        \Pelican_Cache::clean("Frontend/Citroen/NavigationSession");
    ...
    public function cleanTables( $siteId, $langueId, $type )
    {
        ...
        if (is_array($this->aGammeObjects) && !empty($this->aGammeObjects)){
            ...
            foreach( $this->aGammeObjects as $sGammeClassName){                
               ...
               if($siteId && $langueId && $type){
                    $oConnection->query('DELETE FROM ' . $oTableObject->getPHPFactoryTableName(). ' WHERE SITE_ID =' . $siteId . ' AND LANGUE_ID =' . $langueId . ' AND GAMME = "' . $type . '"'); 
                }elseif($siteId && $langueId){
                    $oConnection->query('DELETE FROM ' . $oTableObject->getPHPFactoryTableName(). ' WHERE SITE_ID =' . $siteId . ' AND LANGUE_ID =' . $langueId);
                }elseif($sitesId){
                    $oConnection->query('DELETE FROM ' . $oTableObject->getPHPFactoryTableName(). ' WHERE SITE_ID =' . $siteId );
                }else{
    ...
    */
    ...
    private function importDataToBdd ($oTableObject)
    {
        ...
        if (!empty($sSqlInsert) && $bConcatVersion === true){
            try {
				...
				error_log("Exception SQL : {$sDataClassName} : ".$e->getMessage());
			}
            unset($sSqlInsert);
        }
        $oConnection->commit(); 
     ...
     */
    public function importCSVDataToObject($sitesId, $langueId, $type)
    {        
        /* Initialisation des variables */
        ...
        if (self::isReadableExport($this->sWSExportFilePath) === true) {
            ...
            $this->cleanTables($sitesId, $langueId, $type);
