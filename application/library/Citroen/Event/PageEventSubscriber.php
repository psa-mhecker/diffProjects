...
class PageEventSubscriber implements EventSubscriberInterface {
    ...
    public function onActionTrack($event) {
        ...
        // Scoring referrer
        $_SESSION[APP]['perso_referrer'] = null;
        $referrerScore = \Pelican::$config['PERSO']['BANNER_SCORE'];
        if (!empty($event->params['from-banner'])) {
            $referrerScoreSet = \Pelican_Cache::fetch('Frontend/Citroen/Perso/ReferrerScore', array($_SESSION[APP]['SITE_ID']));
            $defaultScore = isset($referrerScoreSet['default']['SCORE']) ? $referrerScoreSet['default']['SCORE'] : \Pelican::$config['PERSO']['BANNER_SCORE'];
            $matchingScore = isset($referrerScoreSet['keywords'][$event->params['from-banner']]) ? $referrerScoreSet['keywords'][$event->params['from-banner']] : null;
            $referrerScore = $defaultScore;
            if ($matchingScore) {
                $referrerScore = $matchingScore['USE_DEFAULT_SCORE'] == 1 ? $defaultScore : $matchingScore['SCORE'];
                $_SESSION[APP]['perso_referrer'] = array(
                    'keyword' => $matchingScore['KEYWORD'],
                    'score' => $referrerScore,
                );
            }
        }
        ...
        if (!empty($event->params['from-banner'])&&count($aPageProducts)) {
            foreach ($aPageProducts as $key => $val) {
                $aPageProducts[$key]['PRODUCT_PAGE_SCORE'] = max($val['PRODUCT_PAGE_SCORE'], $referrerScore);
            }
            ...
            if($iFoundProduct){
                    ...
                    $aPageProducts[] = array(
                        ...
                        'PRODUCT_PAGE_SCORE' => $referrerScore
                    );
                }   
        }
            ...
            foreach ($aPageProducts as $aOnePageProduct) {
                foreach ($aParams as $sParamName => $mParamValue) {
                    ...
                    if (is_array($mParamValue) && $sParamName == 'values') {
                        ...
                        } elseif ($mParamValue == $aOnePageProduct['lcdv6'] && !empty($mParamValue) ) {
                            $aProductToTouch = $aOnePageProduct;
                            break;
                        }
                    }
