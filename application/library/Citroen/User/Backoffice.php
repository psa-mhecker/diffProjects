 ...
 */
require_once ('Pelican/User/Backoffice.php');
 ...
 */
class Citroen_User_Backoffice extends Pelican_User_Backoffice
{
    ...
    public function login($id, $password, $bIsLdap = false)
    {
        ...
        $authAdapter = new Citroen_Auth_Adapter_Db_Basic();
            ...
            if (is_array($aRights['PROFILES']) && count($aRights['PROFILES']) > 0) {
                ...
                foreach ($aRights['PROFILES'] as $pays => $profiles) {
                    ...
                    if (is_array($aPage) && count($aPage) > 0) {
						...
						WHERE
						CONTENT_ID in (SELECT #pref#_content_version.CONTENT_ID from #pref#_content_version where #pref#_content_version.PAGE_ID in (" . implode(',', $list) . "))
						";
                        $oConnection->query($sqlSaveContent, $this->aBind);
                    }
                    $oConnection->commit();
     ...
     * @return array $aSite
     */
    static private function _getSitesByCode()
    {
        $oConnection = Pelican_Db::getInstance();
        ...
        if (is_array($aRes) && count($aRes) > 0) {
            foreach ($aRes as $res) {
                $aSite[$res['SITE_CODE_PAYS']] = $res['SITE_ID'];
            }
        }
        return $aSite;
    }
     ...
     * @return array $aSite
     */
    static private function _getSites()
    {
        $oConnection = Pelican_Db::getInstance();
     ...
     */
    static private function _getMetier()
    {
        $oConnection = Pelican_Db::getInstance();
     ...
     */
    static private function _getProfilesBySites()
    {
        $oConnection = Pelican_Db::getInstance();
     ...
     * @return array $aRoles
     */
    static private function _getRoles()
    {
        $oConnection = Pelican_Db::getInstance();
     ...
     */
    static private function _getContentTypeBySite()
    {
        $oConnection = Pelican_Db::getInstance();
     ...
     */
    static private function _getRights($aLdapRights)
    {
