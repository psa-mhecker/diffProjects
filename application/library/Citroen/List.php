...
class Citroen_List extends Pelican_List {
     ...
     /**
        ...
        if (!name) {
            ...
            /** cas des checkbox */
                ...
                $filter_script.="event.returnValue = false;
	...
	public function getTable($groupby = false, $bNoRecordText = true, $aBind = array()) {
        ...
        if ($this->aTableValues || $bNoRecordText == false) {
            ...
            if ($this->aTableValues) {
                    ...
                    foreach($aRealTableStructure as $column) {
                        ...
                        if ($bShow) {
                            switch ($column->sColumnType) {
                                ...
                                case "textarea": {
                                            $td_value = self::makeTextarea($column, $values, $id);
                                        break;
                                    }
                                case "image": {
                ...
                $html = $html_css . Pelican_Html::table(array(id => $this->sTableId, width => $this->sTableWidth, cellpadding => $this->iTableCellpadding, cellspacing => $this->iTableCellspacing, border => $this->iTableBorder, 'class' => $this->sTableClass, style => $this->sTableStyle, summary => "Donn�es"), $html . $html_body);
            } else {
                if ($this->bTableHeaders) {
    ...
    static function makeTextarea($column, $values, $id = "") {
        $value = $values[$column->sColumnField];

        $input = Pelican_Html::textarea(array(name => $id, id => $id, 'class' => $column->aColumnAttributes['class'], "onchange" => $column->aColumnAttributes["_onchange"]), $value);

        return $input;
    }

    static function makeInput($column, $values, $id = "") {
        // url
        $checked = false;
        ...
        if (isset($column->aColumnAttributes["_image_"])) {
            ...
            if ($column->sColumnFormat != "button") {
                $input = Pelican_Html::input(array(type => $column->sColumnFormat, value => $value, checked => $checked, onclick => $url, onchange => $column->aColumnAttributes["_onchange"], name => $id, id => $id, 'class' => $column->sColumnFormat));
            } else {
