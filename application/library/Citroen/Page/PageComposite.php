 ...
 */
class Citroen_Page_PageComposite
{
	 ...
	 *
	 */
	public function getPagesParentes($aPages)
	{
		$oConnection = Pelican_Db::getInstance();
		$sSQL = "SELECT DISTINCT p.PAGE_ID FROM #pref#_page p WHERE ";
		foreach ($aPages as $i => $iPage) {
			if ($i > 0) {
				$sSQL .= " OR ";
			}
			$sSQL .= " CONCAT('#', p.PAGE_PATH, '#') LIKE '%#" . $iPage . "#% '";
		}
		return $oConnection->queryTab($sSQL);
	}
	...
	public function save()
	{
		if (!empty($this->aPage)) {
					...
					foreach ($aPageInfo as $langIdCible => $pageInfo) {
						...
						if ($pageIdParentCible) {
							...
							if (isset($idNewsPage) && !empty($idNewsPage)) {
								...
								$oContent = new Citroen_Page_Content($idNewsPage);
			...
			if(isset($pathCible) && !empty($pathCible)){
				$oPage->updatePathParentAndChildren($page['PAGE']['PAGES_ID_RACINE'][0], $pathCible, $page['PAGE']['PAGES_ID_NEWS'][0]);
			}
			unset( $page['PAGE'] );
			if (!isset($aPage['ERROR']) || empty($aPage['ERROR'])) {
				return $aPage;
			}
			return $aError;
		}
