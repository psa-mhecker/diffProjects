...
class Financement
{
		...
		try {
			...
			if ($idSession) {
				$serviceParams = array(
					...
					'vehicleDescription' => $sLabelVehicule, //On ne sait pas où il faut la récupérer
	...
	public static function getCreditPriceML($sPays, $sLanguage, $sDevise, $sLCVD6, $sLabelVehicule, $sDescVehicule = '', $sGammeVehicule, $sPrixHTVehicule, $sPrixTTCVehicule)
	{
		...
		if ($aReturn['DTL'] && ($sPrixHTVehicule || $sPrixTTCVehicule)) {
			...
			$sDTL   = str_replace(array("&amp;", "&"), array("&", "&amp;"), $sDTL);
			...
			$aCreditPriceML['PersoURL'] = $aReturn['PersoURL'];
			$aCreditPriceML['PersoHeight'] = preg_match('/^(\d+)px$/i', $aReturn['PersoHeight'], $matches) ? intval($matches[1]) : null;
			$aCreditPriceML['PersoWidth'] = preg_match('/^(\d+)px$/i', $aReturn['PersoWidth'], $matches) ? intval($matches[1]) : null;
			...
			for ($i = 0; $i < count($xmlDTL->variable); $i++) {
				...
				} else if (!empty($xmlDTL->variable[$i]->id)) {
