            ...
            if ($result) {
                if ($result['REWRITE_RESPONSE'] == '301') {
                    if ($result['REWRITE_TYPE'] == 'PAGE') {
						if(Pelican_Controller::isMobile() && isset($_GET['ppid']) && intval($_GET['ppid'])>0){
							$sUrl = getUrl($result['REWRITE_ID'], "PAGE", $this->params[1], $result['LANGUE_ID']);
							$aAddParam = array('ppid'=>$_GET['ppid']);
							if (strpos($sUrl,'?') !== false) {
								$return['url'] = $sUrl.'&'.http_build_query($aAddParam);
							}else{
								$return['url'] = $sUrl.'?'.http_build_query($aAddParam);
							}
