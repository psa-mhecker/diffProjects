...
class Citroen_GammeVehiculeGamme extends Pelican_Cache {
    ...
    function getValue() {
        ...
        if ( !is_null($this->params[2]) && !empty($this->params[2])){
        ...
        if ( !empty($this->params[3]) ){
         ...
         if ( !empty($this->params[4]) ){
            if(is_array($this->params[4]) && !empty($this->params[4])){
                $gammeFilter = $this->params[4];
            }
...
SQL;
        if(!empty($gammeFilter)){
            foreach ($gammeFilter as $key => $value) {
                $gammeFilter[$key] = (string)$oConnection->strToBind($value);
            }
            $gammeFilter = implode(',',$gammeFilter);
             $sSqlQuery .= <<<SQL
                AND wvg.GAMME IN ($gammeFilter)
SQL;
        }
