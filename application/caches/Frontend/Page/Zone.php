        ...
        if ($tabZones) {
            foreach ($tabZones as $index=>$data) {
                ...
                if ($data["AREA_ID"]) {
                    ...
                    //$data["ZONE_SKIN"] =  'showroom';
                    //$return[$data["AREA_ID"]][]= $data;
                    $return[$data["AREA_ID"]][$index][0]= $data;
                    if($data['ZONE_PERSO']){
                        $arr =  \Citroen_View_Helper_Global::arrHtmlDecode(\Citroen_View_Helper_Global::objectsIntoArray(json_decode($data['ZONE_PERSO'])));
                        if(is_array($arr) && count($arr)>0){
                            foreach($arr as $profil){
                                // Exclusion des profils dont la période de validité ne s'applique pas actuellement
                                if (Citroen_Cache::isPubliePerso($profil) == false) {
                                    continue;
                                }

                                $sProductsId =  $profil['PRODUCT_ID'];
                                if(is_array($profil['PRODUCT_ID']) && count($profil['PRODUCT_ID']) > 1){
                                    $sProductsId =  implode(":", $profil['PRODUCT_ID']);
                                }elseif(is_array($profil['PRODUCT_ID'])){
                                    $sProductsId =  $profil['PRODUCT_ID'][0];
                                }
                                $sKey =  ($profil['INDICATEUR_ID'] && $profil['PRODUCT_ID']) ? $profil['PROFILE_ID'].'_'.$profil['INDICATEUR_ID'].'_'. $sProductsId : $profil['PROFILE_ID'];
                                if(!isset($return[$data["AREA_ID"]][$index][$sKey])){
                                    $return[$data["AREA_ID"]][$index][$sKey] = $profil;
                                }
                            }
                        }
                    }
            ...
            foreach ($tabMultiZones as $index=>$data) {
                ...
                if ($data["AREA_ID"]) {
                    ...
                    if($data['ZONE_PERSO']){
                        ...
                        if(is_array($arr) && count($arr)>0){
                            foreach($arr as $profil){
                                $sKey =  ($profil['INDICATEUR_ID'] && $profil['PRODUCT_ID']) ? $profil['PROFILE_ID'].'_'.$profil['INDICATEUR_ID'].'_'.$profil['PRODUCT_ID'] : $profil['PROFILE_ID'];
                                if(!isset($return[$data["AREA_ID"]][$index][$sKey])){
                                    if (isset($profil['MULTI_NAME']) && $profil['MULTI_NAME'] == 'SLIDESHOW_GENERIC') {
                                        if (!isset($profil['ZONE_MOBILE'])) $profil['ZONE_MOBILE'] = 0;
                                        if (!isset($profil['ZONE_WEB']))    $profil['ZONE_WEB']    = 0;
                                    }
                                    $return[$data["AREA_ID"]][$index][$sKey] = $profil;
                                }
                            }
