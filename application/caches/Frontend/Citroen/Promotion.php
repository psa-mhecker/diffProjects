...
class Frontend_Citroen_Promotion extends Pelican_Cache
{
    ...
    function getValue()
    {
        ...
        $aBind[":STATE_ID"] = 1;
        if ($this->params[3] == "CURRENT") {
            $aBind[":STATE_ID"] = 4;
        }
        // conditionner sur le PAGE_STATUS
        // sans contrôler si en mode prévisu
        if ($type_version == 'CURRENT' || $type_version == 'DRAFT') { // mode normal
        ...
        $sSQL = "
			...
			AND p.LANGUE_ID = :LANGUE_ID
                        AND $cond_status
                        AND (pv.STATE_ID = :STATE_ID $forcePublished)
        ...
        for ($i = 0; $i < $iCount; $i++) {
            ...
            $sSQLCTA = "
				...
				ORDER BY pzmm.PAGE_ZONE_MULTI_ORDER";
