...
class Frontend_Citroen_VehiculesNeufs_getVehicles extends Pelican_Cache
{
	...
	function getValue()
	{
		...
		$typeSite = $this->params[11];
		$sDisponibilite = t('DISPO_SOUS');
		$sEconomie = t('SOIT_ECO');
		$aResult = Vehicules::getVehicles($sPays, $sLanguageCode, $iLat, $iLng, $sModelCode, $sBodyCode, $iPage, $nbElements,$typeAff,$imaxDistance, $typeSite);
		$aReturn = array();
		if (is_array($aResult['VehicleType']['GetVehicleType']) && sizeof($aResult['VehicleType']['GetVehicleType']) > 0 && $aResult['CountVehicle'] > 0) {
			...
			foreach ($aReturn['CARS'] as $key => $car) {
				...
				if ($car['InternetPrice'] != 0 && $car['CatalogPrice'] != 0 && $car['InternetPrice'] != $car['CatalogPrice']) {
					...
					if(intval($car['LessThan8yearsRecovery'])>0 && intval($car['MoreThan8yearsRecovery'])>0){
						$aReturn['CARS'][$key]['AIDE_REPRISE'] = max($car['LessThan8yearsRecovery'],$car['MoreThan8yearsRecovery']);
						$sAideReprise = max($car['LessThan8yearsRecovery'],$car['MoreThan8yearsRecovery']);
					}
					if(intval($car['NationalDiscount'])>0 ){
						$aReturn['CARS'][$key]['NationalDiscount'] = $car['NationalDiscount'];
					}
					if(intval($car['LocalDiscount'])>0 ){
						$aReturn['CARS'][$key]['LocalDiscount'] = $car['LocalDiscount'];
					}
					
					 if (intval($sAideReprise) >0 || intval($car['NationalDiscount']) >0 || intval($car['LocalDiscount'])>0) {
						 $sEconomiePrice = max($car['LessThan8yearsRecovery'],$car['MoreThan8yearsRecovery']) + $car['NationalDiscount'] +  $car['LocalDiscount'];
						 $aReturn['CARS'][$key]['SOIT_ECO'] = str_replace('#PRICE#', $sEconomiePrice . ' #CURRENCY#', $sEconomie);
					 }
				}
			}
		}
