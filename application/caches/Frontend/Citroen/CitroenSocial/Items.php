...
class Frontend_Citroen_CitroenSocial_Items extends Pelican_Cache
{
                    ...
                    foreach($socials as $social) {
                        switch ($type) {
                            ...
                            case 'PINTEREST':
                                ...
                                break;
                            case 'INSTAGRAM':
                                $items = $this->getInstagram($social['RESEAU_SOCIAL_ID_COMPTE'], $social['RESEAU_SOCIAL_URL_WEB']);
                                break;
                        }
                        if (!empty($items)) $return = array_merge($return, $items);
                    }
                }
            }
           // shuffle ($return);
            ...
            foreach($aData['data'] as $data) {
                if ($data['type'] != 'status') {
                    ...
                    if ($sXml->images) {
                        ...
                        if ($picture == '') $picture = $aImages[$width];
						unset($aImages);
                    } else {
    ...
    private function getInstagram($channel, $urlInstagram) {
        $sUrl = "http://iconosquare.com/feed/".$channel;
        $ch = curl_init();
        ...
        if(strpos($xml,'<?xml') !== false){
            ...
            if ($xml->channel->item) {
                foreach($xml->channel->item as $entry) {
                    $order = date("YmdHis",strtotime($entry->pubDate->__toString())).'-instagram-'.$channel;
                    
                    //#src="([^"]+)"#
                    //\'/src *= *["\\\']?([^"\\\']*)/i\'#
                    $url = (string)$entry->description;
                    if (preg_match_all('#src=\'([^"]+)\'#', $url, $matches)) {
                        $media = $matches[1];
                    }
                    $post[$order] = array(
                        'network' => array(
