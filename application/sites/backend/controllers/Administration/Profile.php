...
class Administration_Profile_Controller extends Pelican_Controller_Back
{
    ...
    public function editAction()
    {
        ...
        if ($_SESSION[APP]["admin"]) {
            ...
            $_SESSION[APP]['SITE_ID'] = $this->values['SITE_ID'];
    ...
    protected function beforeSave()
    {
		...
		$oConnection = Pelican_Db::getInstance();
		$aBind[':PROFILE_LABEL'] = $oConnection->strtobind(Pelican_Db::$values['PROFILE_LABEL']);
		$aBind[':SITE_ID'] = Pelican_Db::$values['SITE_ID'];
		$sSql = "SELECT * FROM #pref#_profile where PROFILE_LABEL = :PROFILE_LABEL and SITE_ID = :SITE_ID";
		$aProfile = $oConnection->queryRow($sSql,$aBind);
		
		Pelican_Db::$values["PROFILE_ADMIN"] = $aProfile["PROFILE_ADMIN"];
		Pelican_Db::$values["PROFILE_LABEL"] = $aProfile["PROFILE_LABEL"];
		Pelican_Db::$values["PROFILE_ID"]    = $aProfile["PROFILE_ID"];
		Pelican_Db::$values["SITE_ID"]       = $aProfile["SITE_ID"];
