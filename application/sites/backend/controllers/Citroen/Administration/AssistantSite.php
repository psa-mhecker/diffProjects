...
/**
 * Fichier de Citroen_AssistantSite :
 *
...
class Citroen_Administration_AssistantSite_Controller extends Citroen_Controller {
    ...
    protected $decacheBack = array("Frontend/Site", "Backend/ContentType", "frontend_Frontend/Site/Url", array("Backend/Generic", "site"));
...
function check_url_area(obj){
	callAjax({
			type: "POST",
			data: {urls : obj.value},
			url: "/_/Citroen_Administration_AssistantSite/ajaxVerifUrlArea",
...
function check_code(obj){
	callAjax({
			...
			url: "/_/Citroen_Administration_AssistantSite/checkCode"
		});
}
...
';
    ...
    private function getStep1($oForm) {

        $saveJS = $oForm->_sJS;

        $form .= $oForm->createInput("SITE_LABEL", t('Nom'), 255, "", true, '', $this->readO, 100, false, $js_action);
        ...
        //$form .= $oForm->createTextArea ( "SITE_ROBOT_DESK", t ( 'Robots desk' ).' (?)', false, '', "", $this->readO, 10, 100 , false, "", true, "", t("FONCTIONNEMENT_ROBOT_ADMIN"));
        //$form .= $oForm->createTextArea ( "SITE_ROBOT_MOBILE", t ( 'Robots mobile' ).' (?)', false, '', "", $this->readO, 10, 100 , false, "", true, "", t("FONCTIONNEMENT_ROBOT_ADMIN"));
        ...
        $form .= $oForm->createCheckBoxFromList("SITE_ACTIVATION_MON_PROJET", t('Site Mon Projet'), array("1" => ""), false, false, $this->readO, "h");
    ...
    private function getStep2($oForm) {
        ...
        $saveJS = $oForm->_sJS;
        $form .= $oForm->createTextArea("SITE_DNS", t('Available alias') . ' (?)', true, $oConnection->data ["SITE_DNS"], "", $this->readO, 10, 50, false, "", true, "onBlur=check_url_area(this)", t("FONCTIONNEMENT_DNS"));
    ...
    private function getStep3($oForm) {
        ...
        $strSQLList = "SELECT langue_id as id, " . $oConnection->getConcatClause(array("langue_label", "' ('", "langue_translate", "')'")) . " as lib
				FROM #pref#_language
				ORDER BY lib";
        $form .= $oForm->createAssocFromSql($oConnection, "assoc_langue_id", t('Site languages'), $strSQLList, array(), true, true, $this->readO, 5, 250, false, "");
        $form .= $oForm->createInput("SITE_CODE_PAYS", t('CODE_PAYS'), 2, "", true, '', $this->readO, 10, false, "onBlur=check_code(this)");
