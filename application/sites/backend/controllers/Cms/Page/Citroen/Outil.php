...
{   
    ...
    {   
        ...
        $SQL = "
            SELECT VEHICULE_LABEL, VEHICULE_LCDV6_CONFIG, VEHICULE_LCDV6_MANUAL, VEHICULE_LCDV6_MTCFG
            FROM #pref#_vehicule
        ...
        foreach($Values as $OneValue){
            ...
            if (!empty($OneValue['VEHICULE_LCDV6_MTCFG'])) {
                $key = $OneValue['VEHICULE_LCDV6_MTCFG'];
            }
        ...
        $return .= $controller->oForm->createInput($controller->multi . "ZONE_TITRE5", t('DUREE_AVANT_REPLI_S'), 255, "", false, $controller->zoneValues["ZONE_TITRE5"]?$controller->zoneValues["ZONE_TITRE5"]:5, $controller->readO, 50);        
        ...
        $return .= Backoffice_Form_Helper::getOutils($controller, true, true, 3, 5, false ,0,2,$bPageGenral);
        if(!$bPageGenral){  
