...
class Cms_Page_Citroen_Global_Header_Cookie extends Cms_Page_Citroen
{
    ...
    public static function render(Pelican_Controller $oController)
    {
        ...
        /* Type de cookie */
        if($oController->zoneValues['ZONE_TITRE4'] === NULL){
            $oController->zoneValues['ZONE_TITRE4'] = 1;
        }
        $sControllerForm .= $oController->oForm->createRadioFromList(
                $oController->multi . 'ZONE_TITRE4', 
                t('TYPE_DE_COOKIE'), 
                array('1' => "Cookie CPP", '2' => "Cookie Ghostery"), 
                $oController->zoneValues['ZONE_TITRE4'],
                true, 
                $oController->readO);
        
        //pid
        $sControllerForm .= $oController->oForm->createInput($oController->multi . "ZONE_TITRE5", t('PID Ghostery'), 255, "", true, $oController->zoneValues["ZONE_TITRE5"], $oController->readO, 10);
        //cid
        $sControllerForm .= $oController->oForm->createInput($oController->multi . "ZONE_TITRE6", t('CID Ghostery'), 255, "", true, $oController->zoneValues["ZONE_TITRE6"], $oController->readO, 10);
