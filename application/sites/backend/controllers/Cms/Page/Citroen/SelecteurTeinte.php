...
class Cms_Page_Citroen_SelecteurTeinte extends Cms_Page_Citroen {
    ...
    public static function render(Pelican_Controller $oController) {
        ...
        // Prix off si personnalisation
        $sControllerForm .= $oController->oForm->createCheckBoxFromList(
            $oController->multi."ZONE_TITRE6",
            t('CARTOUCHE_PRIX_OFF_PERSONNALISATION'),
            array(1 => ""),
            $oController->zoneValues['ZONE_TITRE6'],
            false,
            $oController->readO
        );
        ...
        // CPW-4306 : Ajout lien sous "Prix à partir de"
        $sControllerForm .= $oController->oForm->showSeparator();
        $sControllerForm .= $oController->oForm->createLabel(t('AJOUT_LIEN'), '');
        $sControllerForm .= $oController->oForm->createInput($oController->multi . "ZONE_TITRE9", t('LIBELLE_LIEN'), 255, "", false, $oController->zoneValues["ZONE_TITRE9"], $oController->readO, 100);
        $sControllerForm .= $oController->oForm->createInput($oController->multi . "ZONE_TEXTE7", t('URL_LIEN'), 255, "internallink", false, $oController->zoneValues["ZONE_TEXTE7"], $oController->readO, 100, false);
        $sControllerForm .= $oController->oForm->createCheckBoxFromList($oController->multi."ZONE_TITRE10", t('AFFICHAGE_WEB'), array('1' => t('WEB')), $oController->zoneValues['ZONE_TITRE10'], false, $oController->readO);
        $sControllerForm .= $oController->oForm->createCheckBoxFromList($oController->multi."ZONE_TITRE11", t('AFFICHAGE_MOB'), array('1' => t('MOBILE')), $oController->zoneValues['ZONE_TITRE11'], false, $oController->readO);
