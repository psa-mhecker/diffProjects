...
class Cms_Content_Controller extends Cms_Controller
{
    ...
    protected function setListModel ()
    {
        ...
        $this->listModel = "SELECT
			...
			CASE
				WHEN v.CONTENT_TYPE_ID=".Pelican::$config['CONTENT_TYPE_ID']['ACTUALITE']." 
			 		THEN ta.THEME_ACTUALITES_LABEL
			 	WHEN v.CONTENT_TYPE_ID=".Pelican::$config['CONTENT_TYPE_ID']['FAQ']." 
			 	  	THEN fr.FAQ_RUBRIQUE_LABEL
			 	ELSE ' '
			 	END AS THEME,
			 	
			v.CONTENT_CODE AS CONTENT_CODE,
			v.CONTENT_TITLE13 AS CONTENT_TITLE13,
			...
			LEFT JOIN #pref#_content_type tc on( v.CONTENT_TYPE_ID = tc.CONTENT_TYPE_ID )
			LEFT JOIN #pref#_content_type_site cts on( tc.CONTENT_TYPE_ID = cts.CONTENT_TYPE_ID AND v.SITE_ID = cts.SITE_ID)
			LEFT JOIN #pref#_site st on ( v.SITE_ID = st.SITE_ID) ";
					$this->listModel .='LEFT JOIN #pref#_theme_actualites ta 
					
					on ( v.SITE_ID = ta.SITE_ID 
            			AND v.LANGUE_ID = ta.LANGUE_ID
            			AND v.CONTENT_CODE = ta.THEME_ACTUALITES_ID ) 
					'; 	
					$this->listModel .='LEFT JOIN #pref#_faq_rubrique fr 
					
					on ( v.SITE_ID = fr.SITE_ID 
           				AND v.LANGUE_ID = fr.LANGUE_ID 
           				AND v.CONTENT_TITLE13 = fr.FAQ_RUBRIQUE_ID ) ';
				
			$this->listModel .= " WHERE
        ...
        if (! empty($_SESSION[APP]["content_search"]["rechercheTheme"])) {
        	
			if(strpos($_SESSION[APP]["content_search"]["rechercheTheme"],'theme')!==false){
				$theme=explode('_',$_SESSION[APP]["content_search"]["rechercheTheme"] );
				$this->aBind[':CONTENT_THEME'] = $oConnection->strToBind($theme[1]);
				$this->aBind[':TYPE']=Pelican::$config['CONTENT_TYPE_ID']['ACTUALITE'];

				$this->listModel .= " AND v.CONTENT_TYPE_ID = :TYPE";
            	$this->listModel .= " AND v.CONTENT_CODE = :CONTENT_THEME
            	 
            	";	
			}
			else{
				$this->aBind[':CONTENT_THEME'] =$_SESSION[APP]["content_search"]["rechercheTheme"];
				$this->aBind[':TYPE']=Pelican::$config['CONTENT_TYPE_ID']['FAQ'];
				$this->listModel .= " AND v.CONTENT_TYPE_ID = :TYPE";
            	$this->listModel .= " AND v.CONTENT_TITLE13 = :CONTENT_THEME 
    ...
    public function listAction ()
    {
        ...
        $table->addColumn(t('THEME'), "THEME", "20", "center", "", "tblheader", "THEME", false, '', 0, 1, 1);
    ...
    public function editAction ()
    {
        ...
        if (! $template["TEMPLATE_PATH"]) {
			...
			AND  #pref#_content_type.CONTENT_TYPE_ID = " . $_GET["uid"];

            $template = $oConnection->queryRow($sqlTemplate);
        }
        $module_template = $template["TEMPLATE_PATH"];
        $titrePage = $template["CONTENT_TYPE_LABEL"];
        ...
        include_once(dirname(__FILE__) . '/Content/Common/Zone.php');
        $form .= Cms_Content_Common_Pelican_Index_Frontoffice_Zone::render($this);
		*/
		...
		/*$form .= "<script type=\"text/javascript\">
						$('textarea[name=REWRITE_REDIRECT_URL],#CONTENT_CLEAR_URL').focusout(function(){
							callAjax({
								type: \"POST\",
								data: {urls : $(this).val(), id : '".$this->values["CONTENT_ID"]."', field : 'CONTENT_CLEAR_URL', langue : '".$this->values["LANGUE_ID"]."'},
								url: '/_/Citroen_Administration_Url/ajaxVerifUrl'
							});
						});

                        //Controle le changement de titre et ajoute l'ancienne url en redirection
                        //récupération du titre avant changement
                        var beforeTitle = '';
                        $('#CONTENT_TITLE').focusin(function(){
                            beforeTitle = $('#CONTENT_TITLE').val();
						});

                        $('#CONTENT_TITLE').focusout(function(){
                            //vériification si le titre a changé
                            if(beforeTitle != $('#CONTENT_TITLE').val()){
                                //confirmation de récupération d'url
                                if (confirm('". t("ADD_REDIRECTION_301") ."')) {
                                    var concatArea = '';
                                    if($('textarea[name=\"REWRITE_REDIRECT_URL\"]').val() == ''){
                                       concatArea = $('#CONTENT_CLEAR_URL').val();
                                    }else{
                                       concatArea = $('textarea[name=\"REWRITE_REDIRECT_URL\"]').val();
                                       concatArea += '\\n';
                                       concatArea += $('#CONTENT_CLEAR_URL').val();
                                    }

                                    //si l'url n'est pas déjà présente
                                   if(!checkUrlExist($('#CONTENT_CLEAR_URL').val())){
                                        $('textarea[name=\"REWRITE_REDIRECT_URL\"]').val(concatArea);
                                   }
                                }
                            }
						});

                        function checkUrlExist(newLine){
                            var lines = $('textarea[name=\"REWRITE_REDIRECT_URL\"]').val().split('\\n');
                            var exist = false;
                            $.each(lines, function(){
                              if(this == newLine){
                                exist = true;
        ...
        /** commentaires */
       /* if ($_GET['id'] != "-2") {
            $this->object_id = $_GET['id'];
            $this->object_type_id = 1;
            $form .= $this->oForm->beginTab("99");
            include_once (dirname(__FILE__) . '/Content/Common/Comment.php');
            $form .= Cms_Content_Common_Comment::render($this);
        }*/
        // s'il n'y a pas d'onglet : endFormTable();
        $obj1 = "document.fForm.CONTENT_START_DATE";
        $heure1 = "document.fForm.CONTENT_START_DATE_HEURE";

        $obj2 = "document.fForm.CONTENT_END_DATE";
        $heure2 = "document.fForm.CONTENT_END_DATE_HEURE";

        $strFunction = " return checkDatePublication(" . $obj1 . ", " . $obj2 . ", " . $heure1 . ", " . $heure2 . ") ";
    ...
    protected function init ()
    {
        if ($this->getRequest()->isGet()) {
            ...
            if (! isset($_GET["navPage"]) || $_GET["sid"]) {
                ...
                if (isset($_GET["rechercheTheme"])) {
                    $_SESSION[APP]["content_search"]["rechercheTheme"] = $_GET["rechercheTheme"];
                }
    ...
    protected function _getSqlCore ()
    {
        ...
        if(isset($_GET['popup_content']) && $_GET['popup_content']==1){
            $sql .= ' AND IF(cv.CONTENT_START_DATE IS NULL, FALSE, cv.CONTENT_START_DATE < now())' 
