...
class Backoffice_Button_Helper
{
    ...
    public static function init($aButtons = "", $bFront = false)
    {
        ...
        if ($aButtons) {
            /** javascript de la page fille => définition des actions et des boutons à afficher dans la fenêtre parente */
            /*   $display = $aButtonsDef;
			$display .= "function addLoadEvent(func) {
			...
			// lancé uniquement quand la page est complètement chargée : quand typeof window.onload == 'function'
			if (top.displayButtons) {
			if (document.all) {
			...
			top.blocSubmit = false;
			}";
			$display = Pelican_Html::script(array(type => "text/javascript"), $display);
			*/
            ...
            self::$showButtons = Pelican_Html::script(array(
    ...
    public static function display($id, $label, $icon = "", $action = "", $sDisplay = "none")
    {
        if ($action) {
            ...
            $return = Pelican_Html::button(array(
                id => $id,
    ...
    public static function updateButtons($bFront = false)
    {
        ...
        if (!$bFront) {
            ...
            $return .= Pelican_Html::script("if (typeof(CheckForm) == 'function') {
				CheckFormO = CheckForm;
				CheckForm = function(obj) {
					if (CheckFormO(obj)) {
						return true;
...
function clickButton(command, type) {	
	var vSubmit=true;
...
";
        ...
        $return .= "case 'delete' :
			case 'save' : {                                
				...
				if(oForm.FIELD_BLANKS.value == '1' && command != 'state_5'){
					vSubmit =false;
					alert('" . t("FIELD_REQUIRED_BLANKS", 'js') . "');
				}else if(vSubmit == false && oForm.FIELD_BLANKS.value == '0'){
					vSubmit =true;
				}				
				if(command == 'state_5'){
					var sMessage = confirm('" . t('CONFIRM_DELETE_PAGE', 'js') . "');
					if (sMessage == false) {
						return false;
					}else{
                                            vSubmit = true;
				...
				if (type == 'schedule') { 
                                    if(oForm.PAGE_START_DATE_SCHEDULE.value == '' || oForm.PAGE_END_DATE_SCHEDULE.value == ''){
			...
			case 'schedule' : {
				if (confirm('" . t("CONFIMATION_PLANIFICATION", 'js') . "')) {
					blnSchedule = true;
					clickButton('save','schedule');
				}
				break;
			}
			case 'close_schedule' : {
				if (confirm('" . t("CONFIMATION_FERMETURE_MODE_EDITION_PAGE_PLANIFIER", 'js') . "')) {
                                    iframe = document.getElementById('iframeRight');
                                    var url = iframe.contentWindow.location.href;
                                    url = url.replace('?','&'); 
                                    paramsUrl = extractUrlParams(url);                                 
                                    iframe.contentWindow.location.href = \"/_/Index/child?tid=\"+paramsUrl['tid']+\"&id=\"+paramsUrl['id']+\"&view=&scheduleFormClose=1\";
                                    top.showLoading('#frame_right_middle',true);
				}
				break;
			}                      
