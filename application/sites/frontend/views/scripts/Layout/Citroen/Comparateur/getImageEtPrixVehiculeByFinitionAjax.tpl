...
<div class="prices">
	...
	{if $hasCreditPrice == true && $prixCredit neq ''}{'OU_A_PARTIR_DE'|t} <em><strong>{$prixCredit}</strong> {$priceType|t}</em>{if $mLCredit neq ''}<a class="tooltip pop" href="#mLCredit{$idBloc}" {gtmjs type='toggle'  action='Display::ToolTip|' eventGTM='click'   data=$aParams  datasup=['eventLabel'=>{'OU_A_PARTIR_DE'|t}]}>?</a>{/if}<br>{/if}
</div>
