						...
						<div class="row of4 item {$aData.ZONE_SKIN}">
							...
							<div class="col cost">
								...
								{if $car.InternetPrice neq 0 && $car.InternetPrice neq $car.CatalogPrice}
									...
									{/if}
									{if $car.LocalDiscount neq '' || $car.NationalDiscount neq '' || $car.AIDE_REPRISE neq ''}
									<div class="legalsTbaleWrapper"><table class="legals">
									  <tbody>
									  {if $car.LocalDiscount}<tr>
										<td align="left"> {'REMISE_LOCALE'|t}</td>
										<td align="right"> {$car.LocalDiscount}</td>
									  </tr>{/if}
									   {if $car.NationalDiscount}<tr>
										<td align="left"> {'REMISE'|t}</td>
										<td align="right"> {$car.NationalDiscount}</td>
									  </tr>{/if}
									  {if $car.AIDE_REPRISE}<tr>
										<td align="left"> {'AIDE_REPRISE'|t}</td>
										<td align="right">{$car.AIDE_REPRISE}</td>
									  </tr>{/if}
									</tbody></table></div>{/if}
								{/if}
	...
	{elseif $aVehicules.VN|@sizeof > 0 || $aVehicules.LOW_KM|@sizeof > 0}
		...
		<div class="tabbed">
			...
			{if $aVehicules.VN|@sizeof > 0}
				<div id="resultVN" class="tab clsvehiculeneuf ">
					<p class="subtitle tabtitle  masterVn"><span>VÉHICULES NEUFS</span></p>
					<div id='allNewCar' class="stocks">
						<input type="hidden" name="iCount" id="iCount" value="2"/>
						<input type="hidden" name="zid" id="zid" value="{$aData.ZONE_ID}"/>
						<input type="hidden" name="ztid" id="ztid" value="{$aData.ZONE_TEMPLATE_ID}"/>
						{foreach from=$aVehicules.CARS item=car name=listeCar}
							{if $car.StockLevel!=60}
								<div class="row of4 item {$aData.ZONE_SKIN}">
									<figure class="col" {if $car.POURCENTAGE && $active && $car.StockLevel!=60}data-minus="-{$car.POURCENTAGE}%"{/if}>
										<img class="lazy" src="{$car.IMAGE}" data-original="{$car.IMAGE}" width="206" height="116" alt="{$car.CommercialLabel}" />
										<noscript><img src="{$car.IMAGE}" width="206" height="116" alt="{$car.CommercialLabel}" /></noscript>
									...
									<div class="col span2 detail">
										<h4 class="parttitle" {if ($aData.SECOND_COLOR|count_characters)==7 } style="color:{$aData.SECOND_COLOR};" {/if}>{$car.CommercialLabel}</h4>
										<p>
											{'COLOR'|t} : {$car.ExtFeatureLabel}<br />
											{'GARNISSAGE'|t} : {$car.IntFeatureLabel}<br />
										</p>
										...
										{/if}
										<div class="legal legaltext">
											<p>
												{'CONSO_MIXTE'|t} ({$sConso}): {$car.ConsoMixte}<br />
												{'EMISSION_CO2_FRONT'|t} ({$sEmission}) : {$car.CO2Rate} {if $active}<img src="{Pelican::$config.MEDIA_HTTP}/design/frontend/images/picto/{$car.BILAN_CARBONE}.gif" width="25" height="12" alt="A" />{/if}<br /><p>{$car.RecoveryMention}</p>
											</p>
										</div>
										{if $car.NearestDealerName}
											<div class="adress legal">
												<p>
													{$car.NearestDealerName}<br/>
													{$car.NearestDealerAddress1} {$car.NearestDealerAddress2}<br/>
													{$car.NearestDealerZipCode} {$car.NearestDealerCity}
												</p>
											</div>
										{/if}
									</div>
									<div class="col cost">
										<div class="price" {if ($aData.PRIMARY_COLOR|count_characters)==7 } style="color:{$aData.PRIMARY_COLOR};" {/if}>{if $car.InternetPrice}{$car.PRIX_WEBSTORE_STRING|number_format:0:".":" "} {$sDevise} {'CASH_PRICE_TTC'|t} {if $sMentionsLegales}<sup>{$car.PricebackLink}</sup>{/if}{else}{$car.PRIX_CATALOGUE_STRING} {if $sMentionsLegales}<sup>{$car.PricebackLink} </sup>{/if}{/if}</div>
										...
										{if $car.InternetPrice neq 0 && $car.InternetPrice neq $car.CatalogPrice}
											...
											{if $car.SOIT_ECO}<div class="economy">{$car.SOIT_ECO}</div>{/if}
											{if $car.LocalDiscount neq '' || $car.NationalDiscount neq '' || $car.AIDE_REPRISE neq ''}
									<div class="legalsTbaleWrapper"><table class="legals">
									  <tbody>
									  {if $car.LocalDiscount}<tr>
							...
							{if $car.StockLevel==60}
								<div class="row of4 item {$aData.ZONE_SKIN}">
									<figure class="col" {if $car.POURCENTAGE && $active && $car.StockLevel!=60}data-minus="-{$car.POURCENTAGE}%"{/if}>
										...
										<figcaption>
											<p {if ($aData.PRIMARY_COLOR|count_characters)==7 } style="color:{$aData.PRIMARY_COLOR};" {/if}>{$car.DISPONIBLE_SOUS}</p>
											<div class="infosup">{'VEHICULE_FAIBLE_KILOMETRAGE'|t}</div>
										</figcaption>
									</figure>
									<div class="col span2 detail">
										<h4 class="parttitle" {if ($aData.SECOND_COLOR|count_characters)==7 } style="color:{$aData.SECOND_COLOR};" {/if}>{$car.CommercialLabel}</h4>
