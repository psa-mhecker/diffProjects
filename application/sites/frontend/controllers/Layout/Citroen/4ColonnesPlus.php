...
class Layout_Citroen_4ColonnesPlus_Controller extends Pelican_Controller_Front
{
    ...
    $aData['CTA']['ADD_CSS'] = 'buttonLead';
            ...
            ////multi COLONNES
            $multiValuesColonnes = Citroen_Cache::fetchProfiling($aData['ZONE_PERSO'],"Frontend/Citroen/ZoneMulti", array(
                $aData['PAGE_ID'],
                $aData['LANGUE_ID'],
                Pelican::getPreviewVersion(),
                $aData['ZONE_TEMPLATE_ID'],
            ...
            if(isset($multiValuesColonnes)){
                foreach($multiValuesColonnes as $key => $aColonne){
                	//Cta outil 1
                	$ctaOutil = Pelican_Cache::fetch("Frontend/Citroen/BarreOutils", array(
                	$aData['SITE_ID'], $aData['LANGUE_ID'], array($aColonne['PAGE_ZONE_MULTI_ATTRIBUT'])));
                	
                	$index=$aColonne['PAGE_ZONE_MULTI_ATTRIBUT'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_TITRE"]=$ctaOutil[$index]['BARRE_OUTILS_TITRE'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_URL_WEB"]=$ctaOutil[$index]['BARRE_OUTILS_URL_WEB'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_URL_MOBILE"]=$ctaOutil[$index]['BARRE_OUTILS_URL_MOBILE'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_MODE_OUVERTURE"]=$ctaOutil[$index]['BARRE_OUTILS_MODE_OUVERTURE']==1?'self':'blank';
                	
                	
                	//Cta outil 2
                	$ctaOutil = Pelican_Cache::fetch("Frontend/Citroen/BarreOutils", array(
                	$aData['SITE_ID'], $aData['LANGUE_ID'], array($aColonne['PAGE_ZONE_MULTI_ATTRIBUT2'])));
                	
                	$index=$aColonne['PAGE_ZONE_MULTI_ATTRIBUT2'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_TITRE2"]=$ctaOutil[$index]['BARRE_OUTILS_TITRE'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_URL_WEB2"]=$ctaOutil[$index]['BARRE_OUTILS_URL_WEB'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_URL_MOBILE2"]=$ctaOutil[$index]['BARRE_OUTILS_URL_MOBILE'];
                	$multiValuesColonnes[$key]["BARRE_OUTILS_MODE_OUVERTURE2"]=$ctaOutil[$index]['BARRE_OUTILS_MODE_OUVERTURE']==1?'self':'blank';
                	
                    $mediaDetailCol = Pelican_Cache::fetch("Media/Detail", array(
                        $aColonne["MEDIA_ID"]
                    ));
                    $multiValuesColonnes[$key]["MEDIA_PATH"] = $mediaDetailCol["MEDIA_PATH"];
                    $multiValuesColonnes[$key]["MEDIA_TITLE"] = $mediaDetailCol["MEDIA_TITLE"];
                    $multiValuesColonnes[$key]["MEDIA_ALT"] = $mediaDetailCol["MEDIA_ALT"];
                }
            }
            ...
            $this->assign('aColonnes', $multiValuesColonnes);
