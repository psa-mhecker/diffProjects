...
class Layout_Citroen_2ColonnesMixte_Controller extends Pelican_Controller_Front {
    ...
    public function indexAction() {
        ...
        if ($aData["ZONE_WEB"] == 1 || $aData["ZONE_MOBILE"] == 1) {
            ...
            if ($bTrancheVisible) {
                ...
                $this->assign('VIGN_GALLERY_TOP', Pelican_Media::getMediaPath($aData['MEDIA_ID6']));
                ...
                //multi Visuel pour la gallerie dans 2 colonne mixte
                $multiValuesVisuel = Citroen_Cache::fetchProfiling($aData['ZONE_PERSO'], "Frontend/Citroen/ZoneMulti", array(
                            $aData['PAGE_ID'],
                            $aData['LANGUE_ID'],
                            Pelican::getPreviewVersion(),
                            $aData['ZONE_TEMPLATE_ID'],
                ...
                if ($multiValuesVisuel) {
                    
                    if($aData['MEDIA_ID6']){
                        $mediaVisuel = Pelican_Cache::fetch("Media/Detail", array(
                                    $aData["MEDIA_ID6"]
                        ));
                        $VisuelMediaGallerie[] = array(
                            'MEDIA_PATH'=>$mediaVisuel['MEDIA_PATH'],
                            'MEDIA_TITLE'=>$mediaVisuel['MEDIA_TITLE'],
                            'MEDIA_ALT'=>$mediaVisuel['MEDIA_ALT'],
                            'MEDIA_ID'=>$mediaVisuel['MEDIA_ID'],
                        );
                    }
                    
                    foreach ($multiValuesVisuel as $Visuel) {
                    ...
                    //debug($VisuelMediaGallerie);
                    $this->assign('VisuelMediaGallerie', $VisuelMediaGallerie);
                }else{
                    $this->assign('VisuelMediaGallerie', '');
                }
