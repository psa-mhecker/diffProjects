...
class Layout_Citroen_2ColonneSansVisuel_Controller extends Pelican_Controller_Front {
        ...
        if ($aData["ZONE_WEB"] == 1 || $aData["ZONE_MOBILE"] == 1) {
            if (!empty($aData['ZONE_TEXTE'])) {
                if (strpos($aData['ZONE_TEXTE'], '<span>')) {
                    $aData['ZONE_TEXTE'] = str_replace('<p><span>', '<p class="specific"><strong>', $aData['ZONE_TEXTE']);
                    $aData['ZONE_TEXTE'] = str_replace('</span></p>', '</strong></p>', $aData['ZONE_TEXTE']);
                } else {
                    $aData['ZONE_TEXTE'] = str_replace('<p>', '<p class="specific"><strong>', $aData['ZONE_TEXTE']);
