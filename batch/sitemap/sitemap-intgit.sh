...
# Batch de mise à jour des fichiers sitemaps

# On se place dans le rep d'execution de ce script
# Cela evite les dysfonctionnement de cron
cd $(dirname $(readlink -f $0))

echo Déclenchement du batch de mise à jour SITEMAP
...
echo Répertoire de sortie : $(readlink -f $FRONTEND_VAR_PATH)
echo Environnement : $SYMFONY_ENV / $TYPE_ENVIRONNEMENT

op chmod -R 777 $FRONTEND_VAR_PATH/cache
nice -n 10 ../../php.sh -d short_open_tag=on -d memory_limit=2000M ../../frontend/app/console psa:generate:sitemap  --env=$SYMFONY_ENV $*
batch_return_code=$?
if [ $batch_return_code -ne 0 ]
then
	echo C??? : Script PHP SITEMAP a retourné un code erreur $batch_return_code
